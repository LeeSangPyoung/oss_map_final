...
$a.page(function() {
    ...
    function setSelectCode() {
    	...
    	selectList = [ {el: '#hdofcNm', url: 'orgs/'+chrrOrgGrpCd, key: 'orgId', label: 'orgNm'}
                          ...
                          //,{el: '#eqpNm', url: 'eqplist', key: 'comCd', label: 'comCdNm'}
                          ];
        ...
        //최번시
    	//$('#hour').append($('<option>', {value:'busy', text: '최번시'}));
    	for(var i = 0; i <= 23; i++){
    ...
    function setGrid(page, rowPerPage) {
        ...
        if($('#minute5').is(':checked')){
        	param.period = '5min';
        }else if($('#minute15').is(':checked')){
        	param.period = '15min';
        }else{
        	param.period = '1hour';
        }
    	...
    	param.clctDt = date;
    	param.clctHour = $('#hour').val() < 10 ? '0' + $('#hour').val() : $('#hour').val();
    	param.vendCd = selectInit[3].getValue();
    	param.eqpMdlId = selectInit[4].getValue();
    	param.mtsoTypCd = selectInit[5].getValue();
    	param.adstNm = selectInit[6].getText();
    	param.mdlCd = $('#mdlNm').val();
    	param.eqpNm = $("#eqpNm").val();
    	param.mtsoNm = $("#mtsoNm").val();
        //param.clctDtYear = clctDtYear = $('#dateDay').is(':checked') ? date.substring(0,4) : $('#year').val();
        //param.clctDtMon = clctDtMon = $('#dateDay').is(':checked') ? date.substring(4,6) : $('#mon').val();
        //param.clctDtDay = clctDtDay = $('#dateDay').is(':checked') ? date.substring(6,8) : '';

    	httpRequest('tango-transmission-biz/trafficintg/trafficeng/ptsTrafficCurstLkup', param, successCallbackSearch, failCallback, 'GET');
    }
    ...
    function setEventListener() {
        ...
        $('#btnExportExcel').on('click', function(e) {
    		...
    		param.excelUpload = "N";
    		param.method = "ptsTrafficCurstLkup";	// 필수. TrafficExcelBatchDAO 메소드명과 일치시키기
    		...
    		param.fileName = "PtsTrafficCurstLkup" + "_" + dayTime + "." + param.fileExtension;
    		param.excelFlag = "PtsTrafficCurstLkup";     // 필수 값
    		...
    		$('#'+gridId).alopexGrid('showProgress');
    		//param.fileName = "PTS트래픽현황조회";
 	    	//httpRequest('tango-transmission-biz/trafficintg/trafficeng/excelcreatePtsTrafficCurstLkup', param, successCallbackExcel, failCallback, 'GET');
        ...
        $('input[name=time]').on('change', function(e){	$('#btnExportExcel').setEnabled(false);        });
        $('#adstNm').on('change', function(e){        	$('#btnExportExcel').setEnabled(false);        });
        $('#mtsoTyp').on('change', function(e){        	$('#btnExportExcel').setEnabled(false);        });
...
//
//    	selectInit[7] = Tango.select.init({
//    		el: '#eqp',
//    		model: Tango.ajax.init({
...
//    	})
//
//    	selectInit[7].model.get({data:param})
//	}
	...
	var successCallbackSearch = function(response){
		$('#'+gridId).alopexGrid('hideProgress');
		setSPGrid(gridId,response, response.ptsTrafficCurstLkup);
	}
