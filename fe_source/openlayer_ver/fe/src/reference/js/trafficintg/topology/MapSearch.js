...
//    //구성레이어의 조건 값
	...
	//전송실 검색 조건
	var skt_tmof_option_data = [];
	var skb_tmof_option_data = [];

	var gridId5 = "covDsnListGrid";
	var gridId6 = "covListGrid";
	var gridId7 = "covMtsoGrid";
	var gridId7Floor = "covMtsoFloorGrid";

	//layerId
	var covDsnLayer = "MTSO_COV_DSN_LAYER"; //커버리지설계
	var covDsnLayerLabel = "MTSO_COV_DSN_LABEL"; //커버리지설계 라벨
	var hCovDsnLayer = "H_MTSO_COV_DSN_LAYER"; //커버리지 설계 하일라이이트
	var covLayer = "MTSO_COV_LAYER"; //커버리지
	var covLayerLabel = "MTSO_COV_LABEL"; //커버리지 라벨
...
//        		if ( layer.getLayerAliasName()=='T_전송실' ||
...
//	        		mtsoErpLayer.clearLayers();
        ...
        mContainer2.style.height = "770px";
...
//				clearLayerFunc(covLayerLabel);
//				clearLayerFunc(covDsnLayer);
//    			clearLayerFunc(covDsnLayerLabel);
    			clearLayerFunc(hCovDsnLayer);
...
//	    			drawPolygonLabel(selectData, covLayerLabel, gridId6);
				...
				break;
			case 2 :
				var focusedData = AlopexGrid.trimData($('#'+gridId5).alopexGrid("dataGet" , {_state : {focused:true}}));

				if (focusedData.length <= 0 && gridId5FocusInfo.length > 0) {
		    		focusedData = gridId5FocusInfo;
		    	}
        ...
        $('#checkComm').on('click', function(e){
        	if($('#checkComm').is(':checked')){//체크했으면 하위메뉴보여줘
        		...
        		if($('#SKT_BMTSO').is(':checked')){//체크했으면 하위메뉴보여줘
            		...
            		setViewLayerFunc("SKT_BMTSO",true);
        	 	}
        	}else{//체크없앴으면 하위 모드 체크 없애
        		$('#divCommDiv').hide();
...
//        	console.log("aply circleRange : ", parseInt(circleRange));
        	if(parseInt(circleRange) >= 0){
        		var distance_circle_layer = window.distanceCircleLayer;
        		distance_circle_layer.clearLayers();
        		if(parseInt(circleRange) != 0){
...
//				} else if($("#mgmtGrpYn").getValue() == "SKB"){
//					$('#tmofList5').setData({ data:skb_tmof_option_data });
//    			}
			}
		});
...
//				} else if($("#mgmtGrpYn").getValue() == "SKB"){
//					$('#tmofList6').setData({ data:skb_tmof_option_data });
//    			}
			}
		});
    	/*...........................*
        ...
        /**링row를 클릭했을때  링지도표시**/
     	...
     	/** 페이지 번호 클릭시**/
     	 $('#mtsoListGrid').on('pageSet', function(e){
          	var eObj = AlopexGrid.parseEvent(e);
          	setMtsoListGrid(eObj.page, eObj.perPage);
          });
...
//	   		console.log("getEqpList param : ", param.toString());
			$('#eqpListGrid').alopexGrid('showProgress');
 			httpRequest('tango-transmission-biz/transmisson/trafficintg/topology/getEqpList', param.toString(), 'GET', 'searchEqpList');
    	 });
...
//    		console.log("bodycell input click");
    		mgMap.setMode('select'); // 노드편집 모드 제거
    		$('#btnNodeModCancel').hide();
    		$('#btnNodeMod').show();
...
//    		$('#'+gridId5).alopexGrid('updateOption', {rowOption : {inlineStyle: function(data){if(data['coverageId'] == focusedData[0].coverageId) return {color:'red', 'background-color' : 'lightblue'};}}});
    		$('#'+gridId5).alopexGrid("updateOption", { rowOption : {
    																	styleclass : function(data, rowOption){
    																						if(data['coverageId'] == focusedData[0].coverageId){
    																						return 'row-highlight-orange'; } } } });

    		$('#'+gridId5).alopexGrid('viewUpdate');
//    		console.log("gridId5 selectData : ", selectData);
    		if(selectData.length > 0){
    			drawPolygonLayer(selectData, covDsnLayer, gridId5);
    			drawPolygonLabel(selectData, covDsnLayerLabel, gridId5);
    		}
...
//        });
	    ...
	    $('#btnCovReg').on('click', function(e) {
			...
			$a.popup({
				popid : 'CovDsnReg',
				title : '유선 커버리지 정보',
				url : '/tango-transmission-web/trafficintg/topology/CovDsnReg.do',
				data : '',
				modal : true,
				movable : true,
				width : 550,
	    ...
	    $('#btnCovMod').on('click', function(e) {
	    	...
	    	var param = {
	    			coverageId : focusedData[0].coverageId,
	    			coverageTerrNm : focusedData[0].coverageTerrNm,
	    			mtsoId : focusedData[0].mtsoId,
	    			mtsoNm : focusedData[0].mtsoNm,
	    			ldongCd : focusedData[0].ldongCd,
	    			regYn : "Y",
	    	};
    	...
    	$('#btnNodeMod').on('click', function(e){
    		...
    		if(selectData.length > 0){
    			...
    			_.each(selectData, function(row, idx){
           			if(focusedData[0].coverageId == row.coverageId){
           				chkSelData = true;
           			}
           		});
    			...
    			if (focusedData.length == 0 ){
        			callMsgBox('','W', "선택한 영역이 없습니다." , function(msgId, msgRst){});
        			return;
        		}else if (focusedData[0].geoNodeCnt >= 500) {
    				callMsgBox('','W', "선택한 영역의 노드갯수가 500개 이상일 경우 노드편집 할 수 없습니다." , function(msgId, msgRst){});
    				return;
    			}

    			//2023-10-20 노드편집시 이미 선택된 커버리지도 계속 표현되도록 수정
//    			clearLayerFunc(covDsnLayer);
//    			clearLayerFunc(hCovDsnLayer);

    			drawHighlightPolygonLayer(selectData, focusedData, hCovDsnLayer);
    		} else {
    			callMsgBox('','W', "선택한 영역이 없습니다." , function(msgId, msgRst){});
    			return;
    		}
    	...
    	$('#btnNodeSave').on('click', function(e){
    		...
    		httpRequest('tango-transmission-biz/transmisson/trafficintg/topology/updateCoverageDsnGeometry', param, 'POST', 'updateCoverageDsnGeometry');
    	});
...
//			var focusedData = $('#'+gridId6).alopexGrid("dataGet" , {_state : {focused:true}});
//    		$('#'+gridId6).alopexGrid('updateOption', {rowOption : {inlineStyle: function(data){if(data._index.row == focusedData[0]._index.row) return {color:'red', 'background-color' : 'lightblue'};}}});
//    		$('#'+gridId6).alopexGrid("updateOption", { rowOption : {
//																	styleclass : function(data, rowOption){
//																						if(data._index.row == focusedData[0]._index.row){
//																						return 'row-highlight-orange'; } } } });

    		$('#'+gridId6).alopexGrid('viewUpdate');
//    		console.log("gridId6 selectData : ", selectData);
    		if(selectData.length > 0){
    			drawPolygonLayer(selectData, covLayer, gridId6);
    			drawPolygonLabel(selectData, covLayerLabel, gridId6);
    		}
    		...
    		if(selectData.length == 0){
    			...
    			callMsgBox('','I', selectData.length + "개의 영역을 복사합니다." , function(msgId, msgRst){
    				if(msgRst == "Y"){
    		    		...
    		    		httpRequest('tango-transmission-biz/transmisson/trafficintg/topology/copyCoverageGeometry', param, 'POST', 'copyCoverageGeometry');
    				}
    			});
    		}
    	});
    	...
    	//주변국사 및 장비 이벤트 추가
//    	$("input[name=mgmtGrpYn]").on('change', function(e){
//    		var mgmtSelected = $("#mgmtGrpYn").getValue();
...
//			httpRequest('tango-transmission-biz/transmisson/configmgmt/commoncode/eqpRoleDiv/C00148/'+ mgmtSelected, null, 'GET', 'eqpRoleDivCd');
//
//    	});
	};//이벤트리스너 끝
	...
	drawHighlightPolygonLayer = function(layerList, focusRowData, layerId){
		...
		var result = {features: []};
		_.each(layerList, function(feature, idx) {
			if(feature.mtsoId == focusRowData[0].mtsoId){
				var geoJson = L.MG.Util.wktToGeoJSON(focusRowData[0].geo);
				geoJson.properties = feature;
	...
	getPolygonGeometryString = function() {
		...
		var geoString = 'MDSYS.SDO_GEOMETRY( 2003, 8307, NULL, MDSYS.SDO_ELEM_INFO_ARRAY ( 1, 1003, 1 ), MDSYS.SDO_ORDINATE_ARRAY';
		var latlngs = '';
...
// 		console.log("구성 국사 조회 param : ", param);
     		if(clickNm == "bulLayer"){
...
// 			console.log("searchbulLayer param : ", param);
...
//    	console.log("setViewLayerFunc ", layerId, visible);
...
//     	param += '&' + 'mtsoTypCdList=' + '3';
	...
	var setCoverageDsnGrid = function(){
		mgMap.setMode('select'); //노드편집 모드 제거
		clearLayerFunc(covDsnLayer); //커버리지 설계 레이어
		clearLayerFunc(covDsnLayerLabel); //커버리지 설계 라벨 레이어
		clearLayerFunc(hCovDsnLayer); //커버리지 설계 하일라이트 레이어
...
//		gridId5FocusInfo = $('#'+gridId5).alopexGrid('focusInfo');

		gridId5FocusInfo  = AlopexGrid.trimData($('#'+gridId5).alopexGrid("dataGet" , {_state : {focused:true}}));

		$('#'+gridId5).alopexGrid('showProgress');
    	httpRequest('tango-transmission-biz/transmisson/trafficintg/topology/selectCoverageDsnList', param, 'GET', 'selectCoverageDsn');
	}
...
//		console.log("setCoverageGrid : ", param);
		param = convertQueryString(param);
		$('#'+gridId6).alopexGrid('showProgress');
...
//		console.log("onClickFeatures featuresObj : ", featuresObj);
...
//			console.log("layrNm : ", layrNm);
			...
			/*}else if(symbolType == "LineString"){
				...
				httpRequest('tango-transmission-biz/transmisson/trafficintg/topology/getRpetrOfBmtso', param, 'GET', 'searchRpetrOfBmtso');
			}
		}
	}
...
//		console.log("param : ", param.toString());
		...
		httpRequest('tango-transmission-biz/transmisson/trafficintg/topology/getRangeMtsoList', param.toString(), 'GET', 'searchRangeMtsoList');
			    	}
...
//    	console.log("setTab7Data focusedData : ", focusedData);

    	$("#coverageTerrNm7").val(focusedData[0].coverageTerrNm);
    	$("#covMtsoNm7").val(focusedData[0].mtsoNm);
    	$('#tT').progress();
		$('#tA').progress();
		$('#tLn').progress();
		$('#tLnn').progress();
...
//			console.log("rangeMtsoPivotList : ", response.rangeMtsoPivotList);
...
//           	$('#'+gridId5).alopexGrid('focusRestore',gridId5FocusInfo);
    	...
    	if(flag == 'teqpcnt'){
			$('#tab-7').setData(response.teqp[0]);
			$('#tT').progress().remove();
		}
		if(flag == 'aeqpcnt'){
			$('#tab-7').setData(response.aeqp[0]);
			$('#tA').progress().remove();
		}
		if(flag == 'fcltcnt'){
			$('#tab-7').setData(response.fcltlnst[0]);
		}
		if(flag == 'svlncnt'){
			$('#tab-7').setData(response.svln[0]);
			$('#tLn').progress().remove();
		}
		if(flag == 'linecnt'){
			$('#tab-7').setData(response.line[0]);
			$('#tLnn').progress().remove();
		}
		if(flag == 'seqpcnt'){
			$('#tab-7').setData(response.seqp[0]);
			$('#tS').progress().remove();
		}

		if(flag == 'distancecnt'){
//			console.log("distancecnt : ", response.distance);
			$('#'+gridId7).alopexGrid('hideProgress');
			$('#'+gridId7).alopexGrid('dataSet', response.distance);
		}
		if(flag == 'floorSearch'){
//			console.log("floorSearch : ", response.floorList);
...
//    	console.log("clearLayerFunc : ", layerId);
    ...
    function setSelectCode() {
    	...
    	httpRequest('tango-transmission-biz/transmisson/configmgmt/commoncode/tmofs/SKT', null, 'GET', 'tmof'); //전송실
    	httpRequest('tango-transmission-biz/transmisson/configmgmt/commoncode/tmofs/SKB', null, 'GET', 'tmof'); //전송실
        	   ...
        	   id : 'SUBMIT_SETL_PCE_STYLE_POINT_2',
				...
				options: {
	        	   ...
	        	   id : 'SUBMIT_SETL_PCE_STYLE_POINT_3',
					...
					type: L.MG.StyleCfg.STYLE_TYPE.TEXT,
					options: {
