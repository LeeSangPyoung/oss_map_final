...
$a.page(function() {
    ...
    function setRegDataSet(data) {
		...
		if(data.regYn == "Y"){
			...
			//커버리지 확정 관련 추가 필드-2025
			$("#coverageTypCd").val(data.coverageTypCd);
			if("Y" == data.coverageFixYn){
				$("input:radio[name='coverageFixYn']:input[value='Y']").prop('checked', true);
			}else{
				$("input:radio[name='coverageFixYn']:input[value='N']").prop('checked', true);
			}

			//button
//			$("#coverageTypCd").setEnabled(false);
    ...
    function setSelectCode() {
    	...
    	let param = {};
    	param.comGrpCd = "COVTYP"; //커버리지타입

    	httpRequest('tango-transmission-tes-biz/transmisson/tes/commoncode/getCfgCdList', param, 'GET', 'selectCovTypList');
    }
    ...
    function setEventListener() {
    	...
    	//저장
    	 ...
    	 $('#btnCopy').on('click', function(e) {
    		 ...
    		 //저장하시겠습니까?
    		 callMsgBox('','C', "선택한 커버리지를 미확정 상태로 복사하시겠습니까?", function(msgId, msgRst){
    			 //저장한다고 하였을 경우
    			 if (msgRst == 'Y') {
    				 covDsnCopy();
    			 }
    ...
    function covDsnReg() {
		 ...
		 if(param.coverageFixYn == "Y") {
			 param.coverageFixUserId = userId;
		 }else{
			 param.coverageFixUserId = "";
		 }
    ...
    function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'selectCovTypList'){
        	...
        	$('#coverageTypCd').setData({
        		data:option_data,
        		coverageTypCd: (paramData == null) ? "" : paramData.coverageTypCd,
        	});
