...
//    		 alert("엑셀다운버튼 클릭함!!!");
    ...
    function searchHstGrid(param, flag) {     
    	...
    	tangoAjaxModel.get({url : 'tango-transmission-biz/transmission/constructprocess/verification/completionVerificationHistoryLine'
	          , data : param
      		  , flag:flag
...
//					alert('더 이상 조회할 데이터가 없습니다');
					...
					$('#'+gridId).alopexGrid('updateOption',
							{paging : {pagerTotal: function(paging) {
								return '총 건수 : ' + (response.CompletionVerificationHistoryData.vrfCnt);
							}}}
					);
...
//                
//				 '<button id="api_btn" type="button" class="Button button2">(TEST)준공검증이력조회(API)</button>'
//                
//    );
//	
