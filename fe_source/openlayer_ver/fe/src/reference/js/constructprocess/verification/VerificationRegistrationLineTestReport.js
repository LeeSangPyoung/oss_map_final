/**
 * VerificationRegistrationLineTestReport.js
 *
 * @author P096430
...
var p = [
	         {
	        	url:'tango-transmission-biz/transmission/constructprocess/verification/getVerificationRegistrationLineTestReportOtdrList',
	        	flag:'OtdrList' // 0
	         },
	         {
	        	url:'tango-transmission-biz/transmission/constructprocess/verification/getVerificationRegistrationLineTestReportOtdrDetailList',
	        	flag:'otdrDetail' // 1
	         },
	         {
	  			url:'tango-transmission-biz/transmission/constructprocess/verification/getVerificationRegistrationLineTestReportOtdrOpinionInfo',
	  			flag:'opinionResult' // 2
	         },
	         {
	  			url:'tango-transmission-biz/transmission/constructprocess/verification/insertVerificationRegistrationLineTestReportOpinion?method=put',
	  			flag:'saveOpinion' // 3
	         },
	         {  // BP사 의견 및 검증제외 조회
...
//	    	    		$('#otdrListGrid').alopexGrid("dataGetByIndex",{data:AlopexGrid.parseEvent(e).data._index.row}).otdrNo,
//	    	    			""
//    	    			);
//    			}else{
//    				return false;
...
//    		},300);
    	...
    	// 메인그리드 및 상세그리드 더블클릭시 시험측정표 호출
    	// http://localhost:8080/tango-transmission-web/constructprocess/completion/OtdrTestMsmtSave.do

//    	$('#otdrListGrid').on('dblclick','.bodycell',function(e){
//    		otdrTestMgmtSavePopup(
//	    	    		$('#otdrListGrid').alopexGrid("dataGetByIndex",{data:AlopexGrid.parseEvent(e).data._index.row}).otdrNo,
//	    	    		""
//    	    		);
//    	});
   	 ...
   	 // YN Radio Select Event
//     	$('.Radio').on('click',function(e){
//     		setOpinionField($(this).val());
//     	});

	} // setEventListener() - 이벤트


//	function setOpinionField(yn){
// 		if(yn == "N"){
// 			$('#OpR').setEnabled(true);
// 		}else if(yn == "Y"){
// 			$('#OpR').val("");
...
//				if( response.VerificationRegistrationLineTestReportOpinionInfo.cstrVrfRsltCd == 'Y'){
//					$('input:radio[name=r1]:input[value=Y]').parent('label').click();
//				}else if( response.VerificationRegistrationLineTestReportOpinionInfo.cstrVrfRsltCd == 'N'){
//					$('input:radio[name=r1]:input[value=N]').parent('label').click();
//				}
//				$('#OpR').val(response.VerificationRegistrationLineTestReportOpinionInfo.vrfOponCtt);
...
//							setOpinionField(response.VerificationRegistrationLineTestReportOpinionInfo.cstrVrfRsltCd);
