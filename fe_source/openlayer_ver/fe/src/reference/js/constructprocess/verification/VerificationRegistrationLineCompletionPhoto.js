/**
 * VerificationRegistrationLineCompletionPhoto.js
 *
...
//			
//			console.log(result + " : " + $('#clipText').val());
//			
//		},2000);
//		
//	
...
var m = {
			...
			ajaxProp  	: [
				             {  // Search Category
				            	url:'tango-transmission-biz/transmission/constructprocess/verification/verificationRegistrationLineCompletionPhoto',
				            	data:"",
				            	flag:'searchCategory' // 0
				             },
				             {  // Search Photo List
				            	url:'tango-transmission-biz/transmission/constructprocess/verification/getVerificationRegistrationLineCompletionPhotoPhotoInfoList',
				            	data:"",
				            	flag:'searchPhotoList' // 1
				             },
				             {  // Search Opinion
				            	url:'tango-transmission-biz/transmission/constructprocess/verification/getVerificationRegistrationLineCompletionPhotoOpinion',
				            	data:"",
				            	flag:'searchOpinion' // 2
				             },
				             {  // Save Opinion
				            	url:'tango-transmission-biz/transmission/constructprocess/verification/insertVerificationRegistrationLineCompletionPhotoOpinion',
				            	data:"",
				            	flag:'saveOpinion' // 3
				             },
				             {  // BP사 의견 및 검증제외 조회 
...
//				}else{
//					$('tbody').find('.Checkbox:eq('+i+')').parents('tr').find('.Textinput').setEnabled(false);
//					$('tbody').find('.Checkbox:eq('+i+')').parents('tr').find('.Textinput').val("");
//				}
//			} 
//		}else{
...
// 	  		if(( $(this).height() - $("#categoryArea").height() ) > 0){
// 	  			$('#categoryArea').css('overflow-y','scroll');
// 	  		}else{
// 	  			$('#categoryArea').css('overflow-y','');
// 	  		}
// 	  		
// 	  		if(( $(this).width() - $("#categoryArea").width() ) > 0){
// 	  			$('#categoryArea').css('overflow-x','scroll');
// 	  		}else{
// 	  			$('#categoryArea').css('overflow-x','');
// 	  		}
 	  	}); 
...
//    	}
    ...
    function setCategory(){
    	...
    	var flag = 'category';
        Tango.ajax({ url : 'tango-transmission-biz/transmisson/constructprocess/verification/verificationRegistrationLineCompletionPhoto'
                   , data : m.params
                   , method : 'GET'
     	           , flag: flag
    	 })
...
//    	$('#MainPhotoArea').css({'background-color':'#ffccff'});
