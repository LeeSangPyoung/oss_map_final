...
var m = {
			...
			ajaxProp  : [
			             {  
			             	url:'tango-transmission-biz/transmission/constructprocess/verification/getVerificationRegistrationLineModificationInfoSkb',
			             	data:"",
			             	flag:'getModificationInfo'
			              },
			              {  
			             	url:'tango-transmission-biz/transmission/constructprocess/verification/getVerificationRegistrationLineModificationVrfFnshDtSkb',
			             	data:"",
			             	flag:'VrfFnshDt' // 검증완료일조회
			              },
			              {  				
			              	url:'tango-transmission-biz/transmission/constructprocess/verification/updateVerificationRegistrationLineModificationOpinionSaveSkb',
			              	data:"",																									
...
//    		m.globalVar.cstrCd = parent.$('#cstrCd').val();
  ...
  //Grid init
    ...
    function successFn(response, status, jqxhr, flag){
    	...
    	case 'getModificationInfo':   	
    		...
    		if($.TcpUtils.isNotEmpty(response)){
				...
				$('#VerificationRegistrationLineModificationMain').find('tr:eq(12)').children('td:eq(0)').text(setResultYN(m.responseData.ModificationInfo3.picVrfRsltCdG));
				$('#VerificationRegistrationLineModificationMain').find('tr:eq(12)').children('td:eq(1)').text(m.responseData.ModificationInfo3.picVrfOpCttG);
				...
				for(i=0 ;i<13;i++){
	    			...
	    			if(i==12 && $.TcpUtils.isNotEmpty(m.responseData.ModificationInfo3.picVrfOpCttG)){$('#R'+i).val(m.responseData.ModificationInfo3.picVrfOpCttG);}
    ...
    function saveOpinion(){
		...
		m.params.picVrfRsltCdG         = $('input:radio[name=r12]').getValue();
		m.params.picVrfOpCttG       = $('#R12').val();
		if(m.params.picVrfRsltCdG == "N" && m.params.picVrfOpCttG == ""){ parent.parent.callMsgBox('focusInput','I', m.message.vrfOppnCttValidation ); return; }; // 불량시 의견은 필수 입니다.}
    ...
    function paramsValidate(){
    	...
    	if(m.globalVar.eteVrfWoYn !="Y"){
	    	...
	    	if(
				...
				m.responseData.ModificationInfo3.picVrfRsltCdG ==   m.params.picVrfRsltCdG &&
				m.responseData.ModificationInfo3.picVrfOpCttG ==   m.params.picVrfOpCttG 
    		...
    		if( m.responseData.ModificationInfo2.cblStrdVrfRsltCd   == m.params.cblStrdVrfRsltCd &&
				m.responseData.ModificationInfo2.cblStrdOponCtt     == m.params.cblStrdOponCtt &&
				m.responseData.ModificationInfo2.mnftNoVrfRsltCd    == m.params.mnftNoVrfRsltCd &&
				m.responseData.ModificationInfo2.mnftVndrOponCtt    == m.params.mnftVndrOponCtt &&
				m.responseData.ModificationInfo2.lnoVrfRsltCd       == m.params.lnoVrfRsltCd &&
