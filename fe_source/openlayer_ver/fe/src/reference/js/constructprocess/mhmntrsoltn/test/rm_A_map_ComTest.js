...
var zoomBounds = {min: 1, max: 1};
// 지도 표현 한계 영역(독도 표시)
var maxBounds = [[32.55750237326084, 124.4], [38.579344152742955, 132.027656]];
// 전국 지도 설정 좌표(setView 설정값)
/*var nationMapConfig = {zoom : 1, center: [35.601484032128495, 127.9075683370008]};*/
var nationMapConfig = {zoom : 1, center: [37.562203872070235, 126.953492868891428]};
// private
var defaultOpt = {
...
T_a.Map = (function() {
	...
	var mapCreate = function (dongCd, lat, lng, zoomLv, layerLv) {
		...
		MapGeo.create("rmMap", "TANGO-RM", defaultOpt).then(function(map) {
		...
		$(markerInfoList).each(function(i, data) {			
			...
			var divIcon = L.divIcon({
				...
				element.context.onclick = function(element){
					...
					//manholeDash_stat(dongCd,dongNm);
	...
	function manholeDash_stat(dongCd,dongNm){
		
		//전국일때
		var data;	
		
		data = {"ldong": dongCd, "ldongNm":dongNm};		
		data.skAfcoDivCd = $('#skAfcoDivCd').val();	
		data.mhAlmErrCd = $('#mhAlmErrCd').val();		
		parent.$a.navigate('/tango-transmission-web/constructprocess/mhmntrsoltnTest/MhDashBoardTest.do', data);
	};	
	...
	return {
		...
		init: function(dongCd, lat, lng, zoomLv, layerLv){
			mapCreate(dongCd, lat, lng, zoomLv, layerLv); // 맵 생성			
		},
