...
$a.page(function() {
	...
	var paramAlmErrCd = "";
	var paramLdong = ""; // 파라미터로 넘어오는 값이 아니면 필요없음
	var paramLdongNm = "";
...
/*         if("" != param.mhAlmErrCd){
         	paramAlmErrCd = param.mhAlmErrCd;
         }
         
         if("" != param.ldong){
         		paramLdong = param.ldong;
         		paramLdongNm = param.ldongNm;
         	
         	    $("#dongCd").val(paramLdong);
				$("#dongNm").html(paramLdongNm);
				$("#dongNm2").html(paramLdongNm);
				$("#dongNm3").html(paramLdongNm);
         }*/
     	...
     	//코드매핑
     	setSelectByCode('mhAlmErrCd', 'all', 'C03226', setSetSelectByCode,'');  
     	
		//sessionTime 연장
		javascript:void(setInterval(function(){

				var dongCd = $("#dongCd").val();
				var dongNm = $("#dongNm").html();	
				$("#dongCd").val(dongCd);
				$("#dongNm").html(dongNm);
				$("#dongNm2").html(dongNm);
				$("#dongNm3").html(dongNm);
			...
			$('#sessionExpiredTime').click();			
		
		},10000)); //10초마다 조회
    	
    };
    
    function setSetSelectByCode(){    	
    ...
    getMaxClctDt = function () {
    	var orgGrpCd = getOrgGrpCd();    
    		...
    		if (window.mgMap_rm == undefined) {
        		rtnVal = T_a.Map.init("0000000000"); // 지역별 케이블정보 지도 초기화
        		console.log("지도초기화시간 : ", rtnVal);
    		} else {
...
function linkManhole(data) {
	...
	$a.navigate('/tango-transmission-web/constructprocess/mhmntrsoltnTest/MhStatCurstLstTest.do', data);
}

function linkManholeSnsr(data) {
	
	$a.navigate('/tango-transmission-web/constructprocess/mhmntrsoltnTest/MhSnsrStatCurstLstTest.do', data);
}
