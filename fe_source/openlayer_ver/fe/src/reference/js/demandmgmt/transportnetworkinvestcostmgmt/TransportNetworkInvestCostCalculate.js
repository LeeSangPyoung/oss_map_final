...
$a.page(function() {
	...
	//그리드 ID
    var neCostAnalysisResultGrid = 'neCostAnalysisGrid';
    var purpNeCostResultGridId = 'purpNeCostgrid';
    ...
    this.init = function(id, param) {
        ...
        initNeCostAnalysisGrid();
        initPurpNeCostResultGridIdGrid();
        
    	setCombo();
    	setEventListener();
    	
    	$('#purpNeCostExcelDown').hide();
    	$('#planDelete').hide();
    };

  	//Grid 초기화
    function initNeCostAnalysisGrid() {
    	var mapping =  [
			//공통
			{
				width : "40px",
			...
			, {
				...
				align:'right',
				width:'100px',
				title : demandMsgArray['equipmentInvestCost'],
        ...
        gridSearchModel = Tango.ajax.init({
        	url: "tango-transmission-biz/transmisson/demandmgmt/transportnetworkinvestcostmgmt/eqpinvestcostcalculatelist"
    		,data: {
        ...
        //그리드 생성
        $('#'+neCostAnalysisResultGrid).alopexGrid({
        	//extend : ['resultGrid'],
            cellSelectable : true,
  ...
  //Grid 초기화
    function initPurpNeCostResultGridIdGrid() {
    	var mapping =  [
			//공통
			{
				selectorColumn : true,
			...
			/*, {
				...
				align:'right',
				width:'100px',
				title : demandMsgArray['assignBudget'],  배정금액
				render: {type:"string", rule : "comma"}
			}
			, {
				key : 'investCost',
				align:'right',
				width:'100px',
				title : demandMsgArray['designAmount'],   설계금액 
				render: {type:"string", rule : "comma"}
			}
			, {
        ...
        //그리드 생성
        $('#'+purpNeCostResultGridId).alopexGrid({
        	//extend : ['resultGrid'],
            cellSelectable : true,
            autoColumnIndex : true,
            fitTableWidth : true,
    ...
    function setEventListener() {
    	...
    	$('#basicTabs').on("tabchange", function(e, index) {
    		switch (index) {
    			case 0 :
    				tabs = 'neCostAnalysis';
    				$('#'+neCostAnalysisResultGrid).alopexGrid("viewUpdate");
    			...
    			case 1 :
    				tabs = 'purpNeCost';
    				$('#'+purpNeCostResultGridId).alopexGrid("viewUpdate");
    	...
    	$('#demdDivCd').on('change', function(e) {
    		demdDivCdVal = $("#demdDivCd option:selected").val();
        	if($("#demdDivCd option:selected").val() == "104001") {
        		decideReqCntYn = true;
        		selectYearBizCombo('bizDivCd', 'Y', $('#afeYr').val(), 'C00618', '', 'T');
        		initPurpNeCostResultGridIdGrid();
        		$('#'+purpNeCostResultGridId).alopexGrid("dataEmpty");
        	}
        	else if($("#demdDivCd option:selected").val() == "104002") {
    	...
    	$('#bizDivCd').on('change',function(e) {
    		...
    		if($('#bizDivCd').val() != ""){
    			if($("#demdDivCd option:selected").val() == "104001") {
    				selectYearBizCombo('bizDivDetlCd', 'Y', $('#afeYr').val(), $("#bizDivCd").val(), '', 'T', 'E');	// 사업구분 소
            	}
            	else if($("#demdDivCd option:selected").val() == "104002") {
            		selectYearBizCombo('bizDivDetlCd', 'Y', $('#afeYr').val(), $("#bizDivCd").val(), '', 'A', 'E');	// 사업구분 소
            	}
            	else {
            		selectYearBizCombo('bizDivDetlCd', 'Y', $('#afeYr').val(), $("#bizDivCd").val(), '', 'TA', 'E');	// 사업구분 소
            	}
        ...
        $('#search').on('click', function(e) {
        	...
        	if(tabs == "neCostAnalysis") {
        		...
        		gridSearchModel.get({
            		data: dataParam,
        		}).done(function(response,status,xhr,flag){successTansportNetworkInvestCostMgmtCallback(response, 'eqpinvestcostcalculatelist');})
        	  	  .fail(function(response,status,flag){failTansportNetworkInvestCostMgmtCallback(response, 'eqpinvestcostcalculatelist');});
        	}
        	else if(tabs == "purpNeCost") {
        		pageInfo = $('#'+purpNeCostResultGridId).alopexGrid('pageInfo');
        		bodyProgress();
            	dataParam.pageNo = pageInfo.current;
            	dataParam.rowPerPage = 500;
            	TansportNetworkInvestCostMgmtRequest('tango-transmission-biz/transmisson/demandmgmt/transportnetworkinvestcostmgmt/eqpinvestcostcalculatelist', dataParam, 'GET', 'eqpinvestcostcalculatelist');
        	}
        });
        ...
        $('#planDelete').on('click', function(e) {
        	...
        	var list = AlopexGrid.trimData($('#'+neCostAnalysisResultGrid).alopexGrid('dataGet', {_state:{selected:true}}));
...
//        	}
        ...
        //엑셀다운로드
        $('#neCostAnalysisExcelDown').on('click', function(e) {
        	bodyProgress();
        	var dataParam =  $("#searchForm").getData();

        	dataParam = gridExcelColumn(dataParam, neCostAnalysisResultGrid);
    		dataParam.fileName = "장비투자대상분석";
        	dataParam.fileExtension = "xlsx";
        	dataParam.excelPageDown = "N";
        	dataParam.excelUpload = "N";
        	dataParam.mthd = 'neCostAnalysisExcelDown';
        	dataParam.tabs = tabs;
        		
        	//console.log(dataParam);
        	
        	TansportNetworkInvestCostMgmtRequest('tango-transmission-biz/transmisson/demandmgmt/transportnetworkinvestcostmgmt/excelcreate', dataParam, 'GET', 'excelDownload');
        });
        
        $('#purpNeCostExcelDown').on('click', function(e) {
        	bodyProgress();
        	var dataParam =  $("#searchForm").getData();

        	dataParam = gridExcelColumn(dataParam, purpNeCostResultGridId);
    		dataParam.fileName = "목적별장비투자";
        	dataParam.fileExtension = "xlsx";
        	dataParam.excelPageDown = "N";
        	dataParam.excelUpload = "N";
        	dataParam.mthd = tabs;
        ...
        $('#demdDecideRequest').on('click', function(e) {
			...
			$('#'+purpNeCostResultGridId).alopexGrid("endEdit");
        	var dataParam =  $("#searchForm").getData();
        	...
        	var list = AlopexGrid.trimData($('#'+purpNeCostResultGridId).alopexGrid('dataGet', {_state:{selected:true}}));
        	...
        	if(flag) {
        		...
        		callMsgBox('','C', "해당 수요에 선로가 포함되어 있을 경우 모두 진행상태가 변경됩니다.<br>" + demandMsgArray['requestForTransportNetworkInvestCostCalculate'], function(msgId, msgRst){  
                	if (msgRst == 'Y') {
                		bodyProgress();
            			...
            			TansportNetworkInvestCostMgmtRequest('tango-transmission-biz/transmisson/demandmgmt/transportnetworkinvestcostmgmt/updateRequestList'
      				           , updateData
      				           , 'POST'
      				           , 'updateEqpRequestList');
            			
            			
                		//var updataData = list[0];
                		/*
                		var updataData = null;
                		
                		if(list.length > 0) {
                			updataData = list[0];
                			updataData.div = "equip";
                		}
                		else {
    ...
    function successTansportNetworkInvestCostMgmtCallback(response, flag){
    	if(flag == 'eqpinvestcostcalculatelist'){
    		bodyProgressRemove();
    		if(tabs == "neCostAnalysis") {
        		...
        		$('#'+purpNeCostResultGridId).alopexGrid("dataSet", response.lists);
    			$('#'+purpNeCostResultGridId).alopexGrid("startEdit");
    		}
    	}
    		...
    		if(response.demdYn == 'Y') {
    			...
    			TansportNetworkInvestCostMgmtRequest('tango-transmission-biz/transmisson/demandmgmt/transportnetworkinvestcostmgmt/demddeciderequest'
				           , dataParam
				           , 'POST'
				           , 'demddeciderequest');
    		}
        		...
        		if (response.eqpMdlYn == 'N'){
        			checkMsg = demandMsgArray['eqpModelNotExist']; /* 장비모델이 선택되지 않은 수요가 존재합니다. */
        			alertBox('W',checkMsg);
        			return false;
        		}

        		checkMsg = demandMsgArray['demandFixRequestNoData'];  /* 수요확정요청할 계획수요가 없습니다. */
    	...
    	else if(flag == 'demddeciderequest') {
    		bodyProgressRemove();
    	...
    	else if(flag == 'updateEqpRequestList'){
    		bodyProgressRemove();
