...
var main = $a.page(function() {
	...
	this.init = function(id, param) {
		...
		if (param.mtsoMapList != undefined && param.mtsoMapList != null) {
			...
			var tmpList = param.mtsoMapList.split(",");
			for(i=0; i < tmpList.length; i++) {
				mtsoMapListExcel.push(tmpList[i]);
			}
							...
							if (value != undefined && value != null && value != ""){
								...
								for(i=0; i < praFildCd.length; i++) {
									for(var j=0; j < grPraCd.length; j++) {
										if (praFildCd[i].toString() == grPraCd[j].value.toString()) {
...
//
				...
				{ key : 'mtsoId', align:'center', title : '국사ID', width: '120', styleclass : 'font-red',
					...
					editable : function(value, data) {
						if(data.repMtsoYn == 'Y') {
							return '<div><input type="text" readonly style="width:100%;background-color:transparent;border:0 solid black;text-align:center;" value="-" /></div>';
						} else {
							return '<div><input type="text" readonly style="width:100%;background-color:transparent;border:0 solid black;text-align:center;" value="'+value+'" /></div>';
						}
					}
				},
...
//		});
		...
		$('#'+gridId).on('rowInlineEditEnd',function(e){
			...
			data.praFildCd = praFildCd;



			if (typeof data.nbdUpsdShtgRsn) {
				if (data.nbdUpsdShtgRsn != null) {
					data.nbdUpsdShtgRsn = data.nbdUpsdShtgRsn.toString();
				}
			}
			if (data.nbdUpsdShtgRsn != null) {
				var nbdUpsdShtgRsn = data.nbdUpsdShtgRsn.toString().replace(/,/gi, '').replace(/undefined/gi, '').replace(/0/gi, '');
			} else {
				var nbdUpsdShtgRsn = "";
			}
			data.nbdUpsdShtgRsn = nbdUpsdShtgRsn;

			//console.log(data);
			httpRequest('tango-transmission-biz/transmisson/configmgmt/upsdmgmt/setUpdateRoUpsdDemdInf', data, 'POST', 'updateRoUpsdDemdInf');

        });

		$(document).on('click', "[id^='btnMtsoSearch']", function(e){
			var dataObj = null;
    		...
    		var popMtsoEqp = $a.popup({
    			...
    			callback : function(data) {
    				...
    				httpRequest('tango-transmission-biz/transmisson/configmgmt/commonlkup/mtsos', param, 'GET', 'mtsoEdited');
  				}
    		});
		});
...
//		$(document).on('change', "[name^='srch']", function(e){
//			gridHide();
//		});
		//조회
		...
		$('#btnMtsoReg').on('click', function(e) {
			var param = {regYn : 'Y'};
			$a.popup({
			  	popid: 'NewMtsoReg',
			  	title: '투자국사 등록',
			      url: '/tango-transmission-web/configmgmt/upsdmgmt/RoUpsdMtsoRegPop.do',
			      data: param,
			      windowpopup : true,
			      modal: true,
			      movable:true,
		...
		$('#btnMtsoDel').on('click', function(e) {
			...
			if (selectData == null || selectData == undefined) {
				callMsgBox('','W', '삭제할 대상을 선택하세요.' , function(msgId, msgRst){});
			} else {
				var tmpCMsgYn = "N";
				if (selectData.repMtsoYn == "Y") {
					var dataObj = $('#dataGrid').alopexGrid('dataGet',{repMtsoId:selectData.repMtsoId, repMtsoYn : 'N'});
					if (dataObj.length > 0) {
						tmpCMsgYn = "Y";
					}
				}
				//console.log(tmpCMsgYn);
				if (tmpCMsgYn == "Y") {
					callMsgBox('','W', '층(국사)정보가 존재합니다.<br>층(국사)를 먼저 삭제 하여 주시기 바랍니다.' , function(msgId, msgRst){});
				} else {
					callMsgBox('','C', "선택하신 국사 또는 층 국사 정보를 삭제하시겠습니까?<br>투자 국사의 경우 해당 층 모두 삭제됩니다.", function(msgId, msgRst){
		 		        if (msgRst == 'Y') {
	 						var data = {upsdDemdId : selectData.upsdDemdId};
	 						httpRequest('tango-transmission-biz/transmisson/configmgmt/upsdmgmt/setDeleteRoUpsdDemdInf', data, 'POST', 'setDeleteRoUpsdDemdInf');
		 		        }
				    });
				}
			}
		...
		$('#btnSmtsoReg').on('click', function(e) {
			var selectData = $('#'+gridId).alopexGrid('dataGet', {_state: {selected: true}})[0];
			if (selectData == null || selectData == undefined) {
				callMsgBox('','W', '목록에서 국사를 먼저 선택하여 주시기 바랍니다.' , function(msgId, msgRst){});
			} else {
				var param = {regYn : 'Y', repMtsoId : selectData.repMtsoId, repMtsoNm : selectData.mtsoNm};
				$a.popup({
				  	popid: 'NewMtsoReg',
				  	title: '층 국사 등록',
				      url: '/tango-transmission-web/configmgmt/upsdmgmt/RoUpsdSmtsoRegPop.do',
				      data: param,
				      windowpopup : true,
				      modal: true,
				      movable:true,
		...
		$('#btnDataMgmt').on('click', function(e) {
			...
			$a.popup({
			  	popid: 'NewMtsoReg',
			  	title: '이력 관리',
			      url: '/tango-transmission-web/configmgmt/upsdmgmt/RoUpsdDemdInfHst.do',
			      data: param,
			      windowpopup : true,
			      modal: true,
			      movable:true,
...
//		   		/* 엑셀정보     	 */
		   		...
		   		var excelFileNm = 'RO_DEMD_Information_'+dayTime;
		   		param.fileName = excelFileNm;
		   		param.fileExtension = "xlsx";
		   		param.excelPageDown = "N";
		   		param.excelUpload = "N";
		   		param.excelMethod = "getRoUpsdDemdList";
...
//			var emdNm				= response.mtsoInfoList[0].mtsoNm;
		...
		if(flag == 'setDeleteRoUpsdDemdInf'){
			var pageNo			= $('#pageNo').val();
			var rowPerPage		= $('#rowPerPage').val();
			main.setGrid(pageNo,rowPerPage);
		}
	...
	this.setGrid = function(page, rowPerPage){
		...
		//$('#dataGrid').progress().remove();
		//$('#'+gridId).alopexGrid('hideProgress');
		httpRequest('tango-transmission-biz/transmisson/configmgmt/upsdmgmt/getRoUpsdDemdList', param, 'GET', 'search');
	}
