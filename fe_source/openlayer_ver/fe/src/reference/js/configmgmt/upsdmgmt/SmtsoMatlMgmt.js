  ...
  //Grid 초기화
    ...
    function setEventListener() {
    	...
    	//엔터키로 조회
         ...
         $('#execlImportBtn').on('click', function(e){
     		var dataObj = null;
     		dataObj = $('#'+treeId).alopexGrid('dataGet', {_state: {selected:true}})[0];
     		if(dataObj != undefined && dataObj.level == '3') {
     			popup('SmtsoMatlMgmtRegPop', $('#ctx').val()+'/configmgmt/upsdmgmt/SmtsoMatlMgmtRegPop.do', '국소별 자재장비 엑셀 업로드', dataObj);
     		} else {
     			callMsgBox('','I', 'Rack 장비를 업로드 할 층을 선택해 주세요' , function(msgId, msgRst){});
     		}
          });
    	...
    	//엑셀다운
         $('#btnExportExcel').on('click', function(e){
     		...
     		var param =  $("#rackGridForm").getData();
     		...
     		param.excelUpload = "N";
     		param.method = "getSmtsoMatlMgmtList";

     		$('#'+gridId).alopexGrid('showProgress');
     		httpRequest('tango-transmission-biz/transmisson/configmgmt/upsdmgmt/smtsoMatlMgmtExcelcreate', param, 'GET', 'excelDownload');
 		});
	};
	...
	function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'search'){
    		...
    		$('#'+treeId).alopexGrid('dataSet', response.smtsoMatlMgmtTreeList);
    	}

    	// Rack 조회
    	if(flag == 'rack'){
    		$('#'+gridId).alopexGrid('hideProgress');
    		setSPGrid(gridId, response, response.smtsoMatlMgmtList);
    	}
    ...
    this.setTreeGrid = function(){
		...
		httpRequest('tango-transmission-biz/transmisson/configmgmt/upsdmgmt/getSmtsoMatlMgmtTreeList', param, 'GET', 'search');
    }
    ...
    this.setGrid = function(page, rowPerPage, searchLevel, searchCd){
    	...
    	$('#'+gridId).alopexGrid('showProgress');
    	httpRequest('tango-transmission-biz/transmisson/configmgmt/upsdmgmt/getSmtsoMatlMgmtList', param, 'GET', 'rack');
    }
