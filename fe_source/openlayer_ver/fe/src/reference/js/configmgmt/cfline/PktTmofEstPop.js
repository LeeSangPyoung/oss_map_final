    ...
    this.init = function(id, param) {
    	if (! jQuery.isEmptyObject(param) ) {
    		...
    		if(mgmtGrpCdNmStr == "SKB"){
    			mtsoTypeLabel = cflineMsgArray['informationCenter'];
    		}else{
    		...
    		$('#uprOwnNmPop').text(mgmtGrpCdNmStr);
    		$('#lowOwnNmPop').text(mgmtGrpCdNmStr);
    		$('#multiYnPop').setData({multiYn : paramData.multiYn  });
    		$('#multiYnPop').setData({lowMtsoId : paramData.lowMtsoId  });
    		$('#multiYnPop').setData({uprMtsoId : paramData.uprMtsoId  });
    		if(paramData.multiYn != "Y"){
        		$('#pktTrkNoPop').setData({pktTrkNoPop : paramData.pktTrkNoList  });    			
    		}
    	}
  ...
  //Grid 초기화
    ...
    function setRegDataSet() {
    	if(paramData != null && paramData.multiYn == "N"){
	    	var schParam = {"pktTrkNo":paramData.pktTrkNoList};
	    	httpRequest('tango-transmission-biz/transmisson/configmgmt/cfline/packettrunkindex/getPktTrkmtsoLnoInfList', schParam, 'GET', 'getPktTrkmtsoLnoInfList');
    	}
    }
    	...
    	$('#uprTmofIdPop').on('change',function(e){
    		...
    		var tmpMgmt = ""
    	 	if(TmofAllData != null && TmofAllData.length > 0){
    			...
    			if(tmpMgmt=="0001"){
    				uprMgmtGrpNm = "SKT";
    			}else if(tmpMgmt=="0002"){
    				uprMgmtGrpNm = "SKB";
    			}
    	 	}
    		$('#uprOwnNmPop').text(uprMgmtGrpNm);
    	});
    	// 하위국사 전송실
    	...
    	//저장 
    	 $('#btnSave').on('click', function(e) {
    		...
    		var tmofDiv = "";
      		...
      		var uprMtsoId =  $('#uprMtsoId').val();
      		var lowMtsoId = $('#lowMtsoId').val();
...
//    	if( searchDiv == "upr" ||  searchDiv == "low"){
//    		orgId = uprTmofOrgId;
...
//    	console.log("tmofId============0=============" + tmofId);
    ...
    function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'tmofData') {
    		if(response.tmofCdList != null && response.tmofCdList.length>0){
    			...
    			for(m=0; m<response.tmofCdList.length; m++){
    				...
    				if(topoSclCd != "031"){ // 가입자망링이 아닌경우는 해당 관리그룹 전송실만 처리
    	...
    	if(flag == 'getPktTrkmtsoLnoInfList') {
    		if(response.getPktTrkmtsoLnoInfList != null && response.getPktTrkmtsoLnoInfList.length>0){
    			var tmofPathData         =  [];
    			var tmofPath_option_data =  [];
    			for(m=0; m<response.getPktTrkmtsoLnoInfList.length; m++){
    				var dataTmofCd = response.getPktTrkmtsoLnoInfList[m];
    				
    				if(dataTmofCd.jrdtMtsoTypCd == "01"){ //상위국사
    					$('#uprTmofIdPop').val(dataTmofCd.tmofNm);
    					$('#uprMtsoId').val(dataTmofCd.tmofId);
    					...
    					$('#lowTmofIdPop').val(dataTmofCd.tmofNm);
    					$('#lowMtsoId').val(dataTmofCd.tmofId);
    ...
    function mtsoReg() {
    	var param =  $("#mtsoRegForm").getData();
	 	$.extend(param,{"pktTrkNoList":[paramData.pktTrkNoList]});
	 	$('#'+gridMtso).alopexGrid("endEdit", { _state : { editing : true }} );
	 	var dataList = $('#'+gridMtso).alopexGrid('dataGet');
    	$.extend(param,{"mtsoLnoList":dataList});
    	
		httpRequest('tango-transmission-biz/transmisson/configmgmt/cfline/packettrunkindex/updatePktTrkMtsoInfo', param, 'POST', 'updatePktTrkMtsoInfo');
    }
