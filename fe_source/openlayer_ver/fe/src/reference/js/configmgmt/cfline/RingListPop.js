...
$a.page(function() {
	...
	var topoSclCdForRuCore = "";  // RU광코어용 링 종류 제한
	var initParam = null;
    ...
    this.init = function(id, param) {
    	if(param.ntwkLineNm != undefined) { // networkPath 검색 조건 표출 (data : param)
    		...
    		if(nullToEmpty(param.vTmofInfo) != "") {
    			...
    			if(isScrbrNetNtwkLine()){
    				var fdfSearchParam = {
   						"svlnNo" : param.vTmofInfo[0].svlnNo
    				}
    				httpRequest('tango-transmission-biz/transmisson/configmgmt/cfline/ring/selectFdfEqpNm', fdfSearchParam, 'GET', 'selectFdfEqpNm');
    			}
...
//    			$('#searchArea').hide();
    	...
    	// RU광코어의 경우 SMUX, 5G-PON, RINGMUX 만 가능
    	if (nullToEmpty(param.svlnLclCd) == '003' && nullToEmpty(param.svlnSclCd) == '101') {
    		topoSclCdForRuCore = "Y";
    	}
...
//			                { selectorColumn : true, width : '100px' } 
    ...
    function setEventListener() { 
    	...
    	// 노드조회:	장비조회
        $('#btnEqpSearchPop').on('click', function(e) {
        	openEqpPop("eqpIdPop", "eqpNmPop");
        });
    	...
    	// 선택 
    	 $('#btnConfirmPop').on('click', function(e) {
     	 	//var dataObj = AlopexGrid.parseEvent(e).data;
     	 	//var dataObj =  $('#'+gridResult).alopexGrid("dataGet", { _state : { selected : true }});
    		var resultData = null;
    ...
    function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'TopoData') {
    		...
    		var topoSclCdForRuCoreList = [];
    		
    		var paramTopoData = nullToEmpty(setInitParam.topoSclCd);
    		for(var n=0 ; n<response.TopoData.length ;n++) {
    			// RU광코어의 경우 망종류 제한
    			if (topoSclCdForRuCore == "Y") {
    				// RINGMUX, 5G-PON, SMUX
    				if (response.TopoData[n].topoSclCd == "010" || response.TopoData[n].topoSclCd == "033" || response.TopoData[n].topoSclCd == "035" || response.TopoData[n].topoSclCd == "036" || response.TopoData[n].topoSclCd == "042") {
    					topoSclCdForRuCoreList.push({value:response.TopoData[n].topoSclCd, text: response.TopoData[n].topoSclNm});
    				}
    			} else {
        			if(paramTopoData != ""){
    		...
    		$('#topoSclCdPop').clear();
        	$('#topoSclCdPop').setData({data : (topoSclCdForRuCore == "Y" ? topoSclCdForRuCoreList : TopoData)});
    	...
    	if(flag == 'selectFdfEqpNm'){
    		if(response.resultList != null){
    			var fdfEqpNm = response.resultList[0].fdfEqpNm;
    			$('#lftEqpNmPop').val(fdfEqpNm);
    			$('#btnSearchPop').click();
    		}
    	}
    }
     ...
     * 가입자망 회선여부
     */
    function isScrbrNetNtwkLine(){
    	if( typeof initParam.svlnLclCd  != "undefined" && typeof initParam.svlnSclCd  != "undefined" && initParam.svlnLclCd == "004" && initParam.svlnSclCd == "201") {
    		return true;
    	} else {
    		return false;
    	}
    ...
    function setGridData(division){
		...
		if(isNotParamNull) {
	    	...
	    	// 기존조건에 setInitParam.topoSclCd != '010' && setInitParam.topoSclCd != '030' 조건의 의미를 알수 없음
	    	// RU광코어의 경우 무조건 화면에서 선택한 망종류로 죄회하도록 처리함
