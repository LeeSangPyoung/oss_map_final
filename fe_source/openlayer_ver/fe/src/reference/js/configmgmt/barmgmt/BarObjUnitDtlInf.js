...
$a.page(function() {
    ...
    this.init = function(id, param) {
        ...
        httpRequest('tango-transmission-biz/transmisson/configmgmt/barmgmt/barObjUnitInf', param, 'GET', 'barObjUnitInf');
    };
    ...
    function initGrid(strGubun) {
    	...
    	if (strGubun == "N") {
    		...
    		var mappingN =  [{ align:'center', title : '순번', width: '40px', numberingColumn: true },
        		...
        		{ key : 'cardId', align:'center', title : '카드ID', width: '80px' },
        		{ key : 'eqpId', align:'center', title : '장비ID', width: '80px' },
        		{ key : 'eqpNm', align:'center', title : '장비명', width: '180px' },
        		{ key : 'cardSerNoval', align:'center', title : '카드시리얼번호', width: '100px' },
        		{ key : 'cardMdlId', align:'center', title : '카드모델ID', width: '80px' },
        		{ key : 'cardMdlNm', align:'center', title : '카드모델명', width: '80px' },
        		{ key : 'cardNm', align:'center', title : '카드명', width: '80px' },
        		{ key : 'cardStatCd', align:'center', title : '카드상태코드', width: '60px' },
        		{ key : 'cardStatNm', align:'center', title : '카드상태', width: '60px' }];
    	}
...
//             rowInlineEdit : true,
	...
	//컬럼 숨기기
	function gridHide() {
		var hideColList = ['matlStatDivCd', 'cardStatCd','barNo','serNo','cardId','eqpId','cardMdlId'];
   	...
   	//페이지 selectbox를 변경했을 시.
        ...
        $('#hisTabs').on('tabchange', function(e, index, index2){
        	...
        	if (index == 1) {
        		...
        		//TANGO 이력
            	httpRequest('tango-transmission-biz/transmisson/configmgmt/barmgmt/tangoHisInf', paramData, 'GET', 'tangoHisInf');
        	}
    ...
    function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'barObjUnitInf'){

    		$('#searchForm').setData(response.barObjUnitInf[0]);

    		paramData = response.barObjUnitInf[0];
    		//NAMS 이력
        	httpRequest('tango-transmission-biz/transmisson/configmgmt/barmgmt/namsHisInf', paramData, 'GET', 'namsHisInf');
    	}
    	...
    	if(flag == 'tangoHisInf'){
    		...
    		setSPGrid(gridId, response, response.tangoHisInf);
    	}
