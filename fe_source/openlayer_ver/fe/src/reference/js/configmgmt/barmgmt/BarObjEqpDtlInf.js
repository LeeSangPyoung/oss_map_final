...
$a.page(function() {
    ...
    this.init = function(id, param) {
        ...
        httpRequest('tango-transmission-biz/transmisson/configmgmt/barmgmt/barObjEqpInf', param, 'GET', 'barObjEqpInf');
    };
    ...
    function initGrid(strGubun) {
    	...
    	if (strGubun == "N") {
    		...
    		var mappingN =  [{ align:'center', title : '순번', width: '40px', numberingColumn: true },
        		...
        		{ key : 'serNo', align:'center', title : '장비시리얼', width: '100px' },
        		{ key : 'eqpMdlId', align:'center', title : '장비모델ID', width: '80px' },
        		{ key : 'eqpMdlNm', align:'center', title : '장비모델명', width: '80px' },
        		{ key : 'barNo', align:'center', title : '바코드번호', width: '80px' },
        		{ key : 'cstrCd', align:'center', title : '공사코드', width: '60px' },
        		{ key : 'wkrtNo', align:'center', title : '작업지시번호', width: '60px' }];
    	}
...
//             rowInlineEdit : true,
	...
	//컬럼 숨기기
	function gridHide() {
		var hideColList = ['matlStatDivCd', 'cardStatCd','barNo','serNo','eqpId','eqpMdlId'];
   	...
   	//페이지 selectbox를 변경했을 시.
        ...
        $('#hisTabs').on('tabchange', function(e, index, index2){
        	...
        	if (index == 1) {
        		...
        		//TANGO 이력
            	httpRequest('tango-transmission-biz/transmisson/configmgmt/barmgmt/tangoEqpHisInf', paramData, 'GET', 'tangoEqpHisInf');
        	}
    ...
    function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'barObjEqpInf'){

    		$('#searchForm').setData(response.barObjEqpInf[0]);

    		paramData = response.barObjEqpInf[0];
    		//NAMS 이력
        	httpRequest('tango-transmission-biz/transmisson/configmgmt/barmgmt/namsHisInf', paramData, 'GET', 'namsHisInf');
    	}
    	...
    	if(flag == 'tangoEqpHisInf'){
    		$('#'+gridId).alopexGrid('hideProgress');

    		setSPGrid(gridId, response, response.tangoEqpHisInf);
    	}
