...
var pop = $a.page(function() {
    ...
    this.init = function(id, param) {
    	...
    	if(param.closeYn == "N"){
    		...
    		$('#nlnkgChk').attr('checked', false);
    	}

    	if(param.fromPortRegWinPop == "Y"){
    		fromPortRegWinPop = param.fromPortRegWinPop;
    	}
...
//    	httpRequest('tango-transmission-biz/transmisson/configmgmt/commoncode/group/C00148', null, 'GET', 'eqpRoleDivCd');
    ...
    function setRegDataSet(data) {
        	...
        	$("#cstrMgnoLkup").val(data.cstrCd);
        	$("#wkrtNoLkup").val(data.wkrtNo);
     	...
     	//IP장비등록
      	$('#btnIpEqpRegPop').on('click', function(e) {
      		callMsgBox('','I', 'IP장비정보 반영은 운용전환 후 최대 6시간 소요될 수 있습니다.' , function(msgId, msgRst){
				if (msgRst == 'Y') {
					window.open('/tango-inventory-im-web/inventory/ip/eqpm/neweqpm/IpEqpNewReqInfo.do');
				}
			});
         });
    	...
    	//장비타입 선택시 이벤트
    	 ...
    	 $('#'+gridId).on('dblclick', '.bodycell', function(e){
    		 var dataObj = null;
     	 	...
     	 	if(closeYn == "N"){
    	 	}else if(fromPortRegWinPop == "Y"){
    	 		$a.close(dataObj);
    	 	}else{
    	 		dataObj.regYn = "Y";
    	 		dataObj.cstrCd = $("#cstrMgnoLkup").val();
    	 		dataObj.wkrtNo = $("#wkrtNoLkup").val();
//    	 		dataObj.eqpInstlMtsoId = dataObj.mtsoId;

    	 		/* 장비상세정보    	 */
    	 		$a.popup({
    	 			popid: 'EqpReg_20180327',
    	 			title: configMsgArray['equipmentRegistration'],
...
//    	 			iframe: true,
	...
	function EqpReg() {
		 dataParam = {"regYn" : "N", "cstrCd" : $("#cstrMgnoLkup").val(), "wkrtNo" : $("#wkrtNoLkup").val()};
		 /* 장비등록     	 */
		 $a.popup({
           	popid: 'EqpReg_20180327_',
           	title: configMsgArray['equipmentRegistration'],
               url: '/tango-transmission-web/configmgmt/equipment/EqpRegPop.do',
               data: dataParam,
               windowpopup : true,
