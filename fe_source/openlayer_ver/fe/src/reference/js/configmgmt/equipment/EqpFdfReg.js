...
//        		$('#shelfNumberReg').setEnabled(false);
            	...
            	if(data.eqpInstlMtsoIdReg != null || data.eqpInstlMtsoIdReg != undefined && (data.eqpInstlMtsoNmReg == null || data.eqpInstlMtsoNmReg == undefined)){
        			var param = {mtsoId : data.eqpInstlMtsoIdReg}
...
//	    			 callMsgBox('','I', configMsgArray['saveFail']+" \n기존 포트수보다 큰 포트수만 저장이 가능합니다." , function(msgId, msgRst){});
    ...
    function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'EqpFdfReg') {
    		...
    		}else if(response.eqpResult == "FailPortCnt"){
    			callMsgBox('','I', "기존 포트수보다 작은 포트수는 저장할 수 없습니다." , function(msgId, msgRst){});
//    			callMsgBox('','I', configMsgArray['saveFail']+" \n기존 포트수보다 큰 포트수만 저장이 가능합니다." , function(msgId, msgRst){});
    	...
    	if(flag == 'mtsoNmList') {
    		$('#eqpInstlMtsoIdReg').val(response.mtsoLkupList[0].mtsoId);//국사ID
            $('#eqpInstlMtsoNmReg').val(response.mtsoLkupList[0].mtsoNm);//국사명
            var defaultEqpNm = "";
	   		defaultEqpNm = "FDF#"+$('#rackNumberReg').getData().rackNumber+"R-"+$('#shelfNumberReg').getData().shelfNumber+"S_"+response.mtsoLkupList[0].mtsoNm;
	   		$('#eqpNmReg').val(defaultEqpNm);
    	}
