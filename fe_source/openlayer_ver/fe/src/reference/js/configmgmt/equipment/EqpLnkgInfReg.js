...
//    		}
    	...
    	if(flag == 'eqpDcnPort') {
    		...
    		if(eqpLnkgInf.lnkgSystmCd != "TEAMS"){
    			...
    			//연동코드가 TEAMS이고 포트정보가 있을 경우 전송장비-직접접속 탭 선택
    ...
    function eqpLnkgInfReg() {
		...
		{
			if(dcnInfList[0].dcnCreYn == "N"){ //CM_DCN_STD_PORT_INF 값, 수정시 CM_DCN_INF에 이미 저장된 경우는 MAKE_DCN_YN값 없음. 이 경우 임의로 저장시키도록 처리. (<--TEAMS 내용임)
				isMakeDcnYn = false;
			}
			...
			if (($("#eqpTid").val().indexOf("_#") > 0 && $("#eqpMdlId").val() == "DMT0001482") || (isSKB && (isRT || !isMakeDcnYn))){
	    	}else{
