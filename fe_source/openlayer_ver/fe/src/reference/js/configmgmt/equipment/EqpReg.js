...
//    		if (data.eqpRoleDivCd == '18'){					// 링먹스 장비 일떄 장비명 비활성화 - yeseo
//    			$('#eqpNmReg').setEnabled(false);
//    		}else{
// 				$('#eqpNmReg').setEnabled(true);
// 			}
        	...
        	$('#btnShpInfMgmt').setEnabled(false);
    		$('#btnPortInfMgmtReg').setEnabled(false);
        	$('#btnEqpLnkgInfReg').setEnabled(false);
        	$('#btnEqpSctnCurstReg').setEnabled(false);
        	$('#btnNtwkLineCurstLkup').setEnabled(false);
        	$('#btnSrvcLineCurstLkup').setEnabled(false);
...
//    	httpRequest('tango-transmission-biz/transmisson/configmgmt/commoncode/group/C00148', null, 'GET', 'eqpRoleDivCdReg');
    	//소유사업자 조회
//    	httpRequest('tango-transmission-biz/transmisson/configmgmt/commoncode/group/C00290', null, 'GET', 'ownBizrCdReg');
    	//관리팀 조회
//    	httpRequest('tango-transmission-biz/transmisson/configmgmt/commoncode/teams', null, 'GET', 'teamOrgId');
    ...
    function setEventListener() {
    	...
    	//토폴로지
    	 $('#btnTopologyReg').on('click', function(e) {
	    	 dataParam = {"eqpId" : $('#eqpIdReg').val()};
	    	 $a.navigate($('#ctx').val()+'/configmgmt/tnbdgm/TnBdgm.do', dataParam);
         });

    	//포트관리
	   	 $('#btnPortInfMgmtReg').on('click', function(e) {
	   		var param =  $("#eqpRegForm").getData();
	   		param.dumyPortYn = "N";
    		 //popupList('PortInfMgmtPop', $('#ctx').val()+'/configmgmt/portmgmt/PortInfMgmtPop.do', configMsgArray['portCurrentState'], param);
    		 $a.popup({
	   ...
	   //포트정보복사
    	...
    	//장비연동정보
    	 $('#btnEqpLnkgInfReg').on('click', function(e) {
    		 var param =  $("#eqpRegForm").getData();
...
// 	              height : window.innerHeight * 0.8
    	...
    	//변경이력
    	 $('#btnChgHstReg').on('click', function(e) {
    		 var param =  $("#eqpRegForm").getData();
    		 popupList('EqpChgHstLkup', $('#ctx').val()+'/configmgmt/equipment/EqpChgHstLkup.do', configMsgArray['changeHistoryReg'], param);
         });
    	            ...
    	            width : 950,
    	           	...
    	           	callback : function(data) { // 팝업창을 닫을 때 실행
						...
						if(data.eqpMdlId != null && data.eqpMdlId != '' && orgMgmtGrpNm == 'SKT' && $('#eqpMdlIdReg').val() != 'DMT0000009' && $('#eqpMdlIdReg').val() != 'DMB0000009'){
	    	               ...
	    	               if (orgMgmtGrpNm == 'SKB' && $('#eqpMdlIdReg').val() != 'DMT0000009' && $('#eqpMdlIdReg').val() != 'DMB0000009') {
...
//	 				if (codeID.eqpRoleDivCd == '18'){						// 링먹스 장비 일떄 장비명 비활성화 - yeseo
//	 	    			$('#eqpNmReg').setEnabled(false);
//	 	    		}else{
//	 	 				$('#eqpNmReg').setEnabled(true);
//	 	 			}
 			...
 			if(codeID.eqpRoleDivCd == '10') {
 	    		...
 	    		$('#eqpHanNm').setEnabled(true);
// 	    		$('#eqpRmkReg').val("사업자/형태/높이 (예, SKT/사각철탑/45m)");
