...
$a.page(function() {
    ...
    function setRegDataSet(data) {
    	...
    	if(data.mgmtGrpNm == "SKT"){
    		$('#'+gridId).alopexGrid("hideCol", 'scrbSrvcMgmtNo', 'conceal');
    		$('#'+gridId).alopexGrid("hideCol", 'svlnCustNo', 'conceal');
    		$('#'+gridId).alopexGrid("hideCol", 'srvcMgmtNo', 'conceal');
    	}else{
    		$('#'+gridId).alopexGrid("showCol", 'scrbSrvcMgmtNo');	
    		$('#'+gridId).alopexGrid("showCol", 'svlnCustNo');
    		$('#'+gridId).alopexGrid("showCol", 'srvcMgmtNo');
    	}
  ...
  //Grid 초기화
    function initGrid() {
        ...
        $('#'+gridId).alopexGrid({
    		...
    		/*}, { 장비구간ID 	 
				...
				title : configMsgArray['equipmentSectionIdentification'],
				width: '120px'
    		}, { 회선그룹일련번호	 
				key : 'lineLnoGrpSrno', align:'center',
				title : configMsgArray['lineGroupSerialNumber'],
				width: '120px'*/
    		}, {/* 서비스회선번호	 */
				...
				key : 'scrbSrvcMgmtNo', align:'center',
				title : '가입서비스관리번호',
				width: '130px'
			}, {/* 서비스회선고객번호	 */
				...
				key : 'lineStatNm', align:'center',
				title : configMsgArray['lineStatus'],
				width: '120px'
			/*}, {
				key : 'srsLineYn', align:'center',
				title : '중요회선여부',
				width: '120px'
			}, {
				key : 'lineCapaNm', align:'center',
				title : '회선용량',
				width: '120px'*/
			}, {/* 상위국사명          */
    ...
    function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'search') {
    		...
    		setSPGrid(gridId, response, response.eqpSrvcLineAcptCurst);

    		var param =  $("#searchForm").getData();	
    		if (param.eqpRegCheck == 'ExistsSrvcLine') {
        		callMsgBox('','I', configMsgArray['saveFail']+" <br>선택한 장비를 사용하는 서비스회선이 있습니다. <br>서비스회선를 해지하신 후 장비/포트를 철거할 수 있습니다." , function(msgId, msgRst){});
        	}
    	}
    ...
    function setGrid(page, rowPerPage) {
    	...
    	$('#rowPerPage').val(rowPerPage);
		 ...
		 $('#'+gridId).alopexGrid('showProgress');
		 httpRequest('tango-transmission-biz/transmisson/configmgmt/equipment/eqpSrvcLineAcptCurst', param, 'GET', 'search');
    }
