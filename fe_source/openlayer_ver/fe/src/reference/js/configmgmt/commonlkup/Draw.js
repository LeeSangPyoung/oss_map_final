...
var comDraw = $a.page(function() {
    ...
    this.init = function(id, param) {
        ...
        //alert(param.eqpId);
        if(paramData.itemId != null && paramData.itemId != undefined && paramData.itemId != "") {
        	$("#divFloor").css('top', '65px');
    		$("#divRackIn").css('top', '100px');
    		$("#divCanvasWarp").css('top', '65px');

        	$("#dataGridTitle").css("display","none");
        	$("#dataGrid").css("display","none");
    		$("#dataGridPadding").css("display","none");


			httpRequest('tango-transmission-biz/transmisson/configmgmt/upsdmgmt/getParamInfoSet', paramData, 'GET', 'ParamInfoSet');
    	} else {
    		$("#divFloor").removeClass('edivFloor');;
    		$("#divRackIn").removeClass('edivRackIn');
    		//$("#img").removeClass('mdivRackIn');

    		$("#divFloor").addClass('mdivFloor');;
    		$("#divRackIn").addClass('mdivRackIn');
    		//$("#img").addClass('mdivRackIn');
        ...
        var adtnAttrVal = $('#adtnAttrVal', parent.top.document).val();
    	// 환경, 상면관리 권한여부
    ...
    function initGrid() {
    	...
    	var mapping =  [
    		{align:'center', title : 'No.', width: '40', resizing: false, numberingColumn: true },
    		/*{key : 'workGubun', align:'center', title : '관리그룹', width: '70',
			...
			{key : 'mtsoTyp', align : 'center', title : '국사유형', width : '90' },
			{key : 'sisulCd', align : 'center', title : '통시코드', width : '90' },
			{key : 'sisulNm', align : 'center', title : '국사명', width : '200'},*/
			{key : 'floorLabel', align : 'left', title : '층정보', width : '100'},
			{key : 'label', align : 'left', title : '랙명', width : '150'},
			{key : 'sPos', align : 'center', title : '랙시작', width : '60'},
			{key : 'ePos', align : 'center', title : '랙종료', width : '60'},
			{key : 'systemNm', align : 'left',title : '실장장비명', width : '100' },
			{key : 'cstrCd', align : 'center', title : '공사코드', width : '150' },
			{key : 'barCode', align : 'center', title : '바코드', width : '150' },
			{key : 'eqpNm', align : 'left', title : '장비명', width : '150' },
			{key : 'eqpId', align : 'center', title : '장비ID', width : '100' },
			{key : 'eqpTid', align : 'center', title : '장비 TID', width : '100' },
			{key : 'eqpRoleDivNm1', align : 'center', title : '장비용도', width : '90' },
			{key : 'eqpStatNm', align : 'center', title : '상태', width : '90' },
//			{key : 'eqpId', align : 'center', title : '상세보기', width : '90',
...
//				}
//			},
			{key : 'namsMatlNm', align : 'center', title : '장비명', width : '250'},
			{key : 'eqpMdlNm', align : 'center', title : '장비모델명', width : '100'},
			{key : 'eqpRoleDivNm2', align : 'center', title : '장비용도', width : '100'},
			{key : 'erpIntgFcltsNm', align : 'center',title : '장비-통합시설명',width : '200'}
    		];

    	gridModel = Tango.ajax.init({
        	url: "tango-transmission-biz/transmisson/configmgmt/commonlkup/rackin"
    		,data: {
    	        pageNo: 1,
    	        rowPerPage: 100,
    	    }
        ...
        $('#'+gridId).alopexGrid({
        	...
        	, headerGroup: [
				{fromIndex:8, toIndex:12, title:'TANGO'},
				{fromIndex:13, toIndex:16, title:'NAMS'}
				],
        ...
        $('#'+gridRack).alopexGrid({
			...
			columnMapping : [
				{key : 'sPos', align : 'center', title : '시작', width : '50px' },
				{key : 'ePos', align : 'center', title : '종료', width : '50px' },
				{key : 'systemNm', align : 'left', title : '실장명' },
				{key : 'eqpNm', align : 'left', title : '장비명' },
				{key : 'eqpId', align : 'left', title : '장비명', hidden : true}],
				message : {
    ...
    function setRegDataSet(data) {
		...
		httpRequest('tango-transmission-biz/transmisson/configmgmt/commonlkup/floorInfo', data, 'GET', 'floorList');
    }


    function setEventListener() {
    	$(window).resize(function() {
			CanvasResize("WIDTH|^@^|"+$("#divCanvas").width()+"|^@@^|HEIGHT|^@^|700");
		})
    		...
    		var dataObj = null;
     	 	...
     	 	if (dataObj.eqpId != undefined && dataObj.eqpId != null && dataObj.eqpId != "") {
        		...
        		parent.top.comMain.popURL(data);
     	 		...
     	 		alert("Dummy 장비 또는 장비 정보가 없습니다.");
     	 		//callMsgBox('','W', "Dummy 장비 또는 장비 정보가 없습니다." , function(msgId, msgRst){});
     	 	}
     	});

    	$('#'+gridRack).on('dblclick', '.bodycell', function(e){
    		var dataObj = null;
     	 	dataObj = AlopexGrid.parseEvent(e).data;
     	 	dataObj.regYn = "Y";
     	 	if (dataObj.eqpId != undefined && dataObj.eqpId != null && dataObj.eqpId != "") {
     	 		var data = {mtsoEqpGubun : "1", mtsoEqpId : dataObj.eqpId, mtsoEqpNm : dataObj.systemNm}; // mtsoEqpGubun : 0(국사), 1(장비)
        		parent.top.comMain.popURL(data);
     	 	} else {
     	 		alert("Dummy 장비 또는 장비 정보가 없습니다.");
     	 		//callMsgBox('','W', "Dummy 장비 또는 장비 정보가 없습니다." , function(msgId, msgRst){});
    	...
    	$('#ckAcDc').on('change', function(e) {
    		Draw();
		});

    	$('#floorId').on('change', function(e) {

    		$("input:checkbox[id=ckAcDc]").prop("checked", false);
    	...
    	$('#btnDrawEdit').on('click', function(e) {
    		var sisulCd = $("#sisulCd").val();
			var floorId = $("#floorId").val();
    ...
    function successCallback(response, status, jqxhr, flag){
    	...
    	if (flag == "floorList") {
    		...
    		for(var i=0; i < response.floorInfo.length; i++){
    			...
    			if(resObj.mtsoId == paramData.mtsoId) {
					selectId = resObj.floorId;
				}
            	 ...
            	 ******************************************/
        		if (variableNm != undefined && variableNm != null && variableNm != "") {
    				if (variableNm == "floorId" && variableVal == resObj.floorId) {
    					option_data.push(resObj);
    	    			floorStandardSize.push(resObj.floorId+":"+resObj.standardSize);
    	    			if(i == 0) {
    	    				baseScale = resObj.standardSize;
    		...
    		if(selectId == null){
    			if (response.length > 0) {
    				if (response.floorInfo[0].floorId != undefined && response.floorInfo[0].floorId != "")
    					selectId = response.floorInfo[0].floorId;
    			}
    		}

    		$('#floorId').setData({ data : option_data,  floorId:selectId});
    	...
    	if(flag == "AcList") {
			if(status == "success") {
				acArr = [];
				$.each(response.acList, function(i, item){
					//alert(response.acList[i].inId);
					var inId = response.acList[i].inId;
					var outId = response.acList[i].outId;
					...
					if (inKey != undefined && outKey != undefined) {
						...
						acArr.push(oItemIn.Key+":"+oItemOut.Key);
					}
				});
			}
		}
    	...
    	if(flag == "DcList") {
			if(status == "success") {
				...
				$.each(response.dcList, function(i, item){
					var inId = response.dcList[i].inId;
					var outId = response.dcList[i].outId;
					var oPage = GetPage(1);
					var oPanel = GetPanel(oPage, 1);
					if (oPage && oPanel) {
						for (var j = 0; j < oPanel.Items.length; j++) {
					...
					if (inKey != undefined && outKey != undefined) {
						var oItemIn = GetItem(oPanel, inKey);
						var oItemOut = GetItem(oPanel, outKey);
						dcArr.push(oItemIn.Key+":"+oItemOut.Key);
					}
...
//					context.closePath();
					...
					// 선택된 아이템 표시
					if (BoardInfo.Selected.getIsExistItem() && !ckAcDc) {
						if (oPage.Key == BoardInfo.Selected.PageKey && oPanel.Key == BoardInfo.Selected.PanelKey) {
							for (var j = 0; j < BoardInfo.Selected.ItemKeys.length; j++) {
								...
								if (oItem) {
									...
									//console.log("::::::::::"+rackUnitCnt);-----------------------------------
									var paramData = {RackId : oItem.ItemId}
									if (clickItem != oItem.ItemId) {
										clickItem = oItem.ItemId;
										$('#'+gridRack).alopexGrid('showProgress');
										httpRequest('tango-transmission-biz/transmisson/configmgmt/commonlkup/eqp', paramData, 'GET', 'Eqp');
									}
...
//		var nDimL 	= Math.round(oItem.Height * baseScale);
//		var DimensionColor = 'rgba(0,0,0,1)';
//		if (alpha != "1.0"){ //
//			// 치수선 Top
//			context.save();
//			context.translate(trans_cx, trans_cy);
...
//			context.fillText(nDimW, -nW/2 + nW/2, -nH/2 - 17);
//			context.restore();
//			context.closePath();
//
//		}
...
//			context.fillStyle = DimensionColor;
//			context.textAlign = "center";
//			context.fillText(nDimL, -nH/2  + nH/2, -nW/2 - 17);
//
//			context.restore();
//			context.closePath();
//
