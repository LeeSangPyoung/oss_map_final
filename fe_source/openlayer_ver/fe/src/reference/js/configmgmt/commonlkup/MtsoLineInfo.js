...
var comLine = $a.page(function() {
    ...
    this.init = function(id, param) {
    	...
    	paramData = param;
    	$('#mtsoLineLkupForm').setData(param);
    	$('#imgRight').click();

    	var mtsoId = paramData.mtsoId;
    	var sameOfficeChk = ($('#sameOfficeChk').is(':checked'))? "Y":"N";

    	var tmpParam =  {mtsoId:mtsoId, sameOfficeChk:sameOfficeChk};

        $('#'+gridId).alopexGrid('showProgress');
        httpRequest('tango-transmission-biz/transmisson/configmgmt/commonlkup/mtsontwks', tmpParam, 'GET', 'mtsontwks');
    ...
    function resizeContents(){
    	var contentHeight = $("#mtsoLineLkupArea").height();
    ...
    function initGrid(strGubun) {
    	...
    	if (strGubun == "N") {
    		var mappingN =  [{ align:'center', title : '순번', width: '40px', numberingColumn: true },
			    ...
			    //{ key : 'mtsoTyp', align:'center', title : configMsgArray['mobileTelephoneSwitchingOfficeType'], width: '100px' },
			    { key : 'mtsoNm', align:'center', title : configMsgArray['mobileTelephoneSwitchingOfficeName'], width: '150px' },
        		...
        		{ key : 'ntwkLineNm', align:'left', title : '링명', width: '250px' },
        		{ key : 'ntwkTypNm', align:'center', title : '망구분', width: '80px' },
        		{ key : 'topoSclNm', align:'center', title : '망종류', width: '80px' },
    		...
    		var mappingN =  [{ align:'center', title : '순번', width: '40px', numberingColumn: true },
			   ...
			   //{ key : 'mtsoTyp', align:'center', title : configMsgArray['mobileTelephoneSwitchingOfficeType'], width: '100px' },
        		...
        		{ key : 'lineNm', align:'left', title : '서비스회선명', width: '250px' },
        		{ key : 'svlnTypNm', align:'center', title : '대분류', width: '80px' },
        		{ key : 'svlnLclNm', align:'center', title : '소분류', width: '80px' },
        		{ key : 'lineLnoGrpSrno', align:'center', title : '경로번호', width: '80px' },
        		{ key : 'svlnSclNm', align:'center', title : '유형', width: '80px' },
        		{ key : 'svlnStatNm', align:'center', title : '상태', width: '60px' },
        		{ key : 'uprMtsoId', align:'center', title : '상위국ID', width: '130px' },
        		{ key : 'uprMtsoNm', align:'center', title : '상위국', width: '130px' },
        		{ key : 'lowMtsoId', align:'center', title : '하위국ID', width: '130px' },
        		{ key : 'lowMtsoNm', align:'center', title : '하위국', width: '130px' }];
...
//	    				{ click: test },
    ...
    function initGridLno(strGubun) {
        if (strGubun == "N") {
    		var mappingL =  [{ align:'center', title : '순번', width: '40px', numberingColumn: true },
    		...
    		var mappingL =  [{ align:'center', title : '순번', width: '40px', numberingColumn: true },
    			{ key : 'trkNm', align:'center', title : '트렁크', width: '100px', rowspan:true },
			    { key : 'ringNm', align:'center', title : '링', width: '100px', rowspan:true },
			    { key : 'wdmNm', align:'center', title : 'WDM트렁크', width: '100px', rowspan:true },
    			{ key : 'lftMtsoNm', align:'left', title : 'WEST국사', width: '150px'},
    			{ key : 'lftEqpRingDivNm', align:'left', title : 'WEST상하위', width: '100px'},
    			{ key : 'lftEqpNm', align:'left', title : 'WEST장비', width: '300px'},
    			{ key : 'lftPortNm', align:'center', title : 'WEST포트', width: '80px'},
    			{ key : 'lftChnlVal', align:'left',title : 'WEST채널', width: '120px'},
...
//     			useGroupRearrange:true,
	...
	function gridResearch() {
		var ckGubun = $("input:radio[id=rdGubun][value='N']").is(":checked") ? true : false;
		var mtsoId = $("#mtsoId").val();
		var sameOfficeChk = ($('#sameOfficeChk').is(':checked'))? "Y":"N";
		var param = {mtsoId : mtsoId, sameOfficeChk:sameOfficeChk};

		$('#'+gridIdLno).alopexGrid('dataEmpty');
		$('#'+gridId).alopexGrid('showProgress');
		...
		if (ckGubun) {
			...
			initGrid('N');
			httpRequest('tango-transmission-biz/transmisson/configmgmt/commonlkup/mtsontwks', param, 'GET', 'mtsontwks');
		} else {
			$("#serviceLine").show();
			$("#ringInfo").hide();

			initGrid('S');
			httpRequest('tango-transmission-biz/transmisson/configmgmt/commonlkup/mtsosrvcs', param, 'GET', 'mtsosrvcs');
		}
    };
    ...
    function setEventListener() {
    	...
    	$(document).on('click', "[id^='rdGubun']", function(e){
    		gridResearch();
      	 });
    	...
    	$('#'+gridId).on('click', '.bodycell', function(e){
			var dataObj = null;
	  	 	...
	  	 	//var ntwkLineNo = 'N900000240758';
	  	 	//var param = {ntwkLineNo : ntwkLineNo};

	  	 	var ckGubun = $("input:radio[id=rdGubun][value='N']").is(":checked") ? true : false;

	  	    $('#'+gridIdLno).alopexGrid('showProgress');

	  	 	if (ckGubun) {
	  	 		initGridLno('N');
				httpRequest('tango-transmission-biz/transmisson/configmgmt/commonlkup/mtsontwksLno', dataObj, 'GET', 'mtsontwksLno');
			} else {
				initGridLno('S');
				httpRequest('tango-transmission-biz/transmisson/configmgmt/commonlkup/mtsosrvcsLno', dataObj, 'GET', 'mtsosrvcsLno');
			}
    	...
    	$('#'+gridId).on('click', '.bodycell', function(e){
    		...
    		if (ckGubun) {
    			...
    			//setTopology("SRVN", "S000000887421", "(사)금융결제원");
    	...
    	//수용 트렁크 조회 팝업
  		$('#acptTrk').on('click', function(e) {
            ...
            if(param.length == 0){
            	...
            	parent.top.comMain.msgBox('W', '선택된 데이터가 없습니다.');
 				return;
 			}
  	...
  	//선번시각화편집
  	   ...
  	   //선번편집
  		$('#serviceLineInfoPop').on('click', function(e) {
  			...
  			var url = '/tango-transmission-web/configmgmt/cfline/ServiceLineInfoPop.do';
  			var width = 1400;
  			var height = 780;

  			if(param[0].svlnLclCd =="001" && param[0].svlnSclCd == "020"){
  				url = '/tango-transmission-web/configmgmt/cfline/ServiceLineIpTransLineInfoPop.do';
  				width = 1000;
  				height = 300;
  			}
   	...
   	$('#btnMtsoRingExportExcel').on('click', function(e) {
   		var dt = new Date();
		...
		var worker = new ExcelWorker({
			excelFileName : "국사통합정보_링_"+recentYMD,
			sheetList : [{
				sheetName : '국사통합정보',
				$grid : $('#'+gridId)
			}]
   	...
   	$('#btnMtsoLineExportExcel').on('click', function(e) {
   		var dt = new Date();
		var recentY = dt.getFullYear();
		var recentM = dt.getMonth() + 1;
		var recentD = dt.getDate();
		...
		var worker = new ExcelWorker({
			excelFileName : "국사통합정보_회선_"+recentYMD,
			sheetList : [{
				sheetName : '국사통합정보',
				$grid : $('#'+gridId)
			}]
    ...
    function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'mtsontwks') {
    		$('#'+gridId).alopexGrid('hideProgress');
    		$('#'+gridId).alopexGrid('dataSet', response.mtsontwks);
    	...
    	if(flag == 'mtsosrvcs') {
    		$('#'+gridId).alopexGrid('hideProgress');
    		$('#'+gridId).alopexGrid('dataSet', response.mtsosrvcs);
    	...
    	if(flag == 'mtsontwksLno') {
    		$('#'+gridIdLno).alopexGrid('hideProgress');
    		$('#'+gridIdLno).alopexGrid('dataSet', response.mtsontwksLno);
	 	}

    	if(flag == 'mtsosrvcsLno') {
    		$('#'+gridIdLno).alopexGrid('hideProgress');
    		$('#'+gridIdLno).alopexGrid('dataSet', response.mtsosrvcsLno);
	 	}
...
//    	    		linkDataArray = [];
//    			});
    			parent.top.comMain.msgBox('I', '검색 결과가 없습니다.');
    			nodeDataArray = [];
	    		linkDataArray = [];
	 	     	return;
    		}
		...
		for(var i=0; i<sctnData.length; i++){
			...
			if(!((searchData == "serviceLineData" || searchData == "trunkData") && i==0 && sctnData[i].lftEqpNm == "DUMMY")){
				...
				if (dupkey != "" && !(searchData == "serviceLineData" && sctnData[i].lftEqpNm == "DUMMY")) {// 서비스회선 조회 시 DUMMY장비는 제외
					...
					with (sctnData[i]) {
						nodedata[nodecnt++] = {nodekey:frkey, mtsoId:lftMtsoId,mtsoNm:lftMtsoNm,mtsoTypCd:lftMtsoTypCd,eqpId:lftEqpId,eqpNm:lftEqpNm,shortNm:shortname,eqpRoleDivCd:lftEqpRoleDivCd,eqpRoleDivNm:lftEqpRoleDivNm,lineSctnLnoSrno:lftLineSctnLnoSrno,mgmtGrpNm:mgmtGrpNm,source:"",loc:""};
					};
				...
				if (i>0 && i==sctnData.length-1 && nodedata[0].eqpId == sctnData[i].rghtEqpId) {
					...
					if (dupkey != "" && !(searchData == "serviceLineData" && sctnData[i].rghtEqpNm == "DUMMY")) {// 서비스회선 조회 시 DUMMY장비는 제외
						...
						if(sctnData[i].lftEqpId != sctnData[i].rghtEqpId){
							with (sctnData[i]) {
								nodedata[nodecnt++] = {nodekey:tokey, mtsoId:rghtMtsoId,mtsoNm:rghtMtsoNm,mtsoTypCd:rghtMtsoTypCd,eqpId:rghtEqpId,eqpNm:rghtEqpNm,shortNm:shortname,eqpRoleDivCd:rghtEqpRoleDivCd,eqpRoleDivNm:rghtEqpRoleDivNm,lineSctnLnoSrno:rghtLineSctnLnoSrno,mgmtGrpNm:mgmtGrpNm,source:"",loc:""};
							};
						}
