...
var userMenuPop = $a.page(function() {
	...
	function initGrid() {
		...
		$('#'+menuGrid).alopexGrid({
    		...
    		autoResize: true,
			columnMapping: [
				{ selectorColumn : true, width : '30px', resizing : false},
				{ key : 'grpNm', align:'center', title : '그룹',width : '40px', rowDragDrop : true},
				{ key : 'fctInvtItmNm', align:'left', title : '항목 명', rowDragDrop : true},
		...
		$('#'+userGrid).alopexGrid({
			...
			columnMapping: [
				...
				{ key : 'grpNm', align:'center', title : '그룹',width : '30px', rowDragDrop : true},
				{ key : 'fctInvtItmNm', align:'left', title : '항목 명', rowDragDrop : true},
				{ key : 'menuId', align:'center', title : 'menuid', hidden : true}
				],
			message: {
	...
	function setSelectCode() {
		...
		httpRequest('tango-transmission-biz/transmisson/configmgmt/commoncode/cfgStd/FCTMENU', null, 'GET', 'grpCd');	// 그룹코드
//		var userId = $("#userId").val();
//		var paramData = {userId : userId};
...
//		httpRequest('tango-transmission-biz/transmisson/configmgmt/mtsoinvt/getUserMenu', paramData, 'GET', 'userMenu'); 	// 사용자 항목
	...
	function setEventListener() {
		...
		$('#fctInvtItmNm').on('keydown',function(event){
			if(event.keyCode == 13){
				//이벤트 등록
				 $('#searchBtn').click();
			}
		...
		$('#searchBtn').on('click', function(){
			...
			httpRequest('tango-transmission-biz/transmisson/configmgmt/fctinvtmgmt/getMenuInf', paramData, 'GET', 'menuInf'); // 항목
        });
		...
		$('#btnPlus').on('click', function(e) {
        	...
        	for ( var i=0; i<data.length; i++ ) {
        		if( $('#'+userGrid).alopexGrid( 'dataGet' , {'menuId' : data[i].menuId} ).length == 0 ) {
					...
					httpRequest('tango-transmission-biz/transmisson/configmgmt/fctinvtmgmt/setUserMenuInsert', paramData, 'POST', 'saveUserMenu');
        		}
        	}
...
//    		httpRequest('tango-transmission-biz/transmisson/configmgmt/mtsoinvt/getMenuInf', paramData, 'GET', 'menuInf'); 		// 항목
		...
		$('#'+userGrid).on('click','.bodycell', function(e){
    		...
    		if ($(this).hasClass('cell-type-deleteScreen')) {
    			...
    			callMsgBox('','C', "해당 항목을 삭제하시겠습니까?", function(msgId, msgRst){
    				if (msgRst == 'Y') {
    					httpRequest('tango-transmission-biz/transmisson/configmgmt/fctinvtmgmt/setUserMenuDelete', paramData, 'POST', 'saveUserMenu');
    				}
    			});
    		}
		});
		...
		$('#btnUserGrpSave').on('click', function(e) {
			...
			httpRequest('tango-transmission-biz/transmisson/configmgmt/fctinvtmgmt/mergeUserGrp', paramData, 'POST', 'mergeUserGrp'); 	// 사용자 그룹 저장
		...
		$('#btnUserGrpDel').on('click', function(e) {
			...
			callMsgBox('','C', configMsgArray['deleteConfirm'], function(msgId, msgRst){
    			...
    			if (msgRst == 'Y') {
    				...
    				httpRequest('tango-transmission-biz/transmisson/configmgmt/fctinvtmgmt/deleteUserGrp', paramData, 'POST', 'deleteUserGrp'); 	// 사용자 그룹 삭제
		...
		$('#userGrpList').on('change', function(e) {
			...
			if(storgboxId.length > 0) {
				...
				httpRequest('tango-transmission-biz/transmisson/configmgmt/fctinvtmgmt/getMenuInf', paramData, 'GET', 'menuInf'); 		// 항목
				$('#'+userGrid).alopexGrid('showProgress');
				httpRequest('tango-transmission-biz/transmisson/configmgmt/fctinvtmgmt/getUserMenu', paramData, 'GET', 'userMenu'); 	// 사용자 항목
			}
			else {
				$("#btnUserGrpMod").hide();
				$("#btnUserGrpDel").hide();
