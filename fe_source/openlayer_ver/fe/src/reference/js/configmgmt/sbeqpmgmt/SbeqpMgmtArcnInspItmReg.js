var main = $a.page(function() {
	...
	var today = new Date();
	var addData  = { "arcnId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "odrEqpStatCd" : "", "tmprStatCd" : "", "opStatCd" : ""};
	var acceptDate = null;
    		...
    		renderMapping: {
    			"date" :  {
					renderer: function(value, data, render, mapping) {
						var $x
						if(data.arcnId == null || data.arcnId == "" || data.arcnId == undefined) {//+-수정건
				...
				"bigo" :  {
					renderer: function(value, data, render, mapping) {
						...
						if (value == undefined || value == "undefined") {
							if(data.arcnId == '' || data.arcnId == null) {
...
//					}
        	...
        	columnMapping: [{
				...
				key : 'odrEqpStatCd', align:'center', title : '실외기상태', width: '80px',
				render : {
      	    		type : 'string',
				...
				key : 'tmprStatCd', align:'center', title : '온도상태', width: '80px',
				render : {
					type : 'string',
					rule : function(value, data) {
						var render_data = [];
				...
				key : 'opStatCd', align:'center', title : '운용상태', width: '80px',
				render : {
					type : 'string',
					rule : function(value, data) {
						var render_data = [];
    ...
    function datePicker(gridId, cellId, rowIndex, keyValue){
    	$('#' + cellId + '').showDatePicker( function (date, dateStr ) {
    		var insertDate = dateStr.substr(0,4) + "-" + dateStr.substr(4,2) + "-" + dateStr.substr(6,2);
    			$('#' + gridId + '').alopexGrid("cellEdit", insertDate, {arcnId : ""}, keyValue);
		});
    }
    ...
    function setEventListener() {
    	$("#btnRowAdd").on('click', function(e) {//+-수정건
    		...
    		if(data == null || data == "" || data == undefined){
	    		...
	    		if(data[0].arcnId == null || data[0].arcnId == ""){
		        }else{
		        	$('#'+gridId).alopexGrid('dataAdd', inputData, {_index:{row:0}});
		        	$('#'+gridId).alopexGrid("startEdit");
		        	$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$("#btnRowDel").on('click', function(e) {//+-수정건
      		...
      		if(data == null || data == "" || data == undefined){
		        ...
		        if(data[0].arcnId == null || data[0].arcnId == "" || data[0].arcnId == undefined){
		        	$('#'+gridId).alopexGrid('dataDelete',{_index:{row:0}});
		        	if(chk[0].arcnId =="" || chk[0].arcnId == null || chk[0].arcnId == undefined){
		        		$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$('#'+gridId).on('click', '.bodycell', function(e) {
			...
			if (dataObjFoucs[0]._key == "inspStdDate" && (dataObjFoucs[0].arcnId == "" || dataObjFoucs[0].arcnId == null || dataObjFoucs[0].arcnId == undefined) ){
        	...
        	try {
        		...
        		if(dataObj[0]._key != "bigoCol") {
                	if (dataObj[0]._key == "inspStdDate" && dataObj[0]._state.focused && (dataObj[0].arcnId == "" || dataObj[0].arcnId == null || dataObj[0].arcnId == undefined)) {
            			datePicker(gridId, dataObj[0]._index.grid + "-" + dataObj[0]._index.row + "-" + dataObj[0]._index.column, dataObj[0]._index.row, dataObj[0]._key);
                    		...
                    		//+-수정건
                    		if (dataObj[0].arcnId == null || dataObj[0].arcnId == "") {
            			...
            			//+-수정건
            			if(dataObj[0]._state.focused && (dataObj[0].arcnId == "" || dataObj[0].arcnId == null || dataObj[0].arcnId == undefined) && acceptDate != dataObj[0].inspStdDate) {
                			dataObj[0].arcnId = $('#arcnId').val();
                			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpArcnInspItmDupChk',dataObj[0], 'GET', 'dupChk');
                			dataObj[0].arcnId = "";
            			}
            		}
        		}
    	...
    	$('#'+gridId).on('click', "[id=btnDel]", function(e){
    		...
    		callMsgBox('','C', "삭제하시겠습니까?", function(msgId, msgRst){
			       //삭제한다고 하였을 경우
		        if (msgRst == 'Y') {
		        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/deleteSbeqpArcnInspItmInf', dataObj[0], 'POST', 'sbeqpDel');
		        }
	     	});
    	});
  		...
  		$('#'+gridId).on('click', "[id='btnReg']", function(e){
  			...
  			var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
			dataObj[0].arcnId = $('#arcnId').val();
			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpArcnInspItmDupChk',dataObj[0], 'GET', 'dupChkReg');
			dataObj[0].arcnId = "";//+-수정건
		});
  		...
  		$('#btnSearch').on('click', function(e) {
    		var param =  $("#sbeqpMgmtArcnInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpArcnInspItmMgmt', param, 'GET', 'gridSearch');
  		});
	...
	function sbeqpArcnInspItmReg() {
		var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
		var chkNew = "N";//+-수정건
		var userId;
		if(dataObj[0].arcnId == "" || dataObj[0].arcnId == null || dataObj[0].arcnId == undefined){
			chkNew = "Y";
		}
		...
		dataObj[0].lastChgUserId = userId;
	 		 ...
	 		 if (dataObj[0].odrEqpStatCd == "") {
	 			//필수선택 항목입니다.[ 실외기 상태 ]
	 			callMsgBox('','W', '필수선택 항목입니다.[ 실외기 상태 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){
	    			dataObj[0].arcnId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].tmprStatCd == "") {
	 			 //필수선택 항목입니다.[ 온도 상태 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 온도 상태 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){
	    			dataObj[0].arcnId = "";
	    		}
	 			 return;
	 		 ...
	 		 callMsgBox('','C', configMsgArray['saveConfirm'], function(msgId, msgRst){
				       //저장한다고 하였을 경우
			        if (msgRst == 'Y') {
			        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/mergeSbeqpArcnInspItmMgmt', dataObj[0], 'POST', 'sbeqpReg');
			        }else{
			        	if(chkNew == "Y"){//+-수정건
			    			dataObj[0].arcnId = "";
			    		}
	...
	function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'dupChkReg'){	// 등록
    		var result = response.sbeqpArcnInspItmDupYn;
    		if(result) {
    			callMsgBox('','i', '동일한 점검일자에 이미 저장된 이력이 있습니다.', function(msgId, msgRst){});
    			var dataObj = $('#'+gridId).alopexGrid("dataGet", {"inspStdDate":response.sbeqpArcnInspItmDupYn});
    	...
    	if(flag == 'search'){	// 상세 View
    		...
    		$('#sbeqpMgmtArcnInspItmForm').setData(result);
    		var param =  $("#sbeqpMgmtArcnInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpArcnInspItmMgmt', param, 'GET', 'gridSearch');
    	}
    	if(flag == 'gridSearch'){	// 점검항목 리스트
    		...
    		var result = response.sbeqpArcnInspItmMgmtList;
//    		addData  = { "arcnId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "odrEqpStatCd" : "", "tmprStatCd" : "", "opStatCd" : ""};
//    		result.unshift(addData);//+-수정건
	       	$('#'+gridId).alopexGrid('dataSet', response.sbeqpArcnInspItmMgmtList);
