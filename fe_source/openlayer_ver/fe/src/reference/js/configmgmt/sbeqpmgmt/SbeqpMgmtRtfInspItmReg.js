var main = $a.page(function() {
	...
	var today = new Date();
	var addData  = { "rtfId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "ipdCapaValStatCd" : "", "rtfcMdulQutyVal" : "0", "insVcurRVal" : "0", "insVcurSVal" : "0", "insVcurTVal" : "0", "prtVoltVal" : "0", "prtVcurVal" : "0"};
	var acceptDate = null;
...
//					}
    		...
    		headerGroup : [
	               { fromIndex : 3, toIndex : 3, title:'정류모듈'},
	               { fromIndex : 4, toIndex : 6, title:'입력전류'},
	               { fromIndex : 7, toIndex : 8, title:'출력전압/전류'}
	        ],
        	columnMapping: [{
				...
				key : 'ipdCapaValStatCd', align:'center', title : 'IPD용량', width: '80px',
				render : {
      	    		type : 'string',
				...
				key : 'rtfcMdulQutyVal', align:'center', title : '수량', width: '60px', editable: {type:'maskedInput'}
			},  {/* R		 */
				key : 'insVcurRVal', align:'center', title : 'R', width: '60px', editable: {type:'maskedInput'}
			}, {/* S   	 */
				key : 'insVcurSVal', align:'center', title : 'S', width: '60px', editable: {type:'maskedInput'}
			}, {/* T		 */
				key : 'insVcurTVal', align:'center', title : 'T', width: '60px', editable: {type:'maskedInput'}
			}, {/* 출력전압 */
				key : 'prtVoltVal', align:'center', title : '출력전압', width: '60px', editable: {type:'maskedInput'}
			}, {/* 출력전류       	 */
				key : 'prtVcurVal', align:'center', title : '출력전류', width: '60px', editable: {type:'maskedInput'}
			}, {/* 비고 */
    ...
    function setEventListener() {
    	...
    	$('#'+gridId).on('click', '.bodycell', function(e) {
    		...
    		if (dataObjFoucs[0]._key == "inspStdDate" && (dataObjFoucs[0].rtfId == "" || dataObjFoucs[0].rtfId == null || dataObjFoucs[0].rtfId == undefined) ){
        	...
        	try {
        		...
        		if(dataObj[0]._key != "bigoCol") {
        			if (dataObj[0]._key == "inspStdDate" && dataObj[0]._state.focused && (dataObj[0].rtfId == "" || dataObj[0].rtfId == null || dataObj[0].rtfId == undefined)) {
            			...
            			//+-수정건
            			if(dataObj[0]._state.focused && (dataObj[0].rtfId == "" || dataObj[0].rtfId == null || dataObj[0].rtfId == undefined) && acceptDate != dataObj[0].inspStdDate) {
                			dataObj[0].rtfId = $('#rtfId').val();
                			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpRtfInspItmDupChk',dataObj[0], 'GET', 'dupChk');
                			dataObj[0].rtfId = "";
            			}
            		}
        		}
    	...
    	$('#'+gridId).on('click', "[id=btnDel]", function(e){
    		...
    		callMsgBox('','C', "삭제하시겠습니까?", function(msgId, msgRst){
			       //삭제한다고 하였을 경우
		        if (msgRst == 'Y') {
		        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/deleteSbeqpRtfInspItmInf', dataObj[0], 'POST', 'sbeqpDel');
		        }
	     	});
    	});
  		...
  		$('#'+gridId).on('click', "[id='btnReg']", function(e){
  			...
  			var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
			dataObj[0].rtfId = $('#rtfId').val();
			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpRtfInspItmDupChk',dataObj[0], 'GET', 'dupChkReg');
			dataObj[0].rtfId = "";//+-수정건
		});
  		...
  		$('#btnSearch').on('click', function(e) {
    		var param =  $("#sbeqpMgmtRtfInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpRtfInspItmMgmt', param, 'GET', 'gridSearch');
  		});
	...
	function sbeqpRtfInspItmReg() {
		...
		dataObj[0].lastChgUserId = userId;
	 		 ...
	 		 if (dataObj[0].ipdCapaValStatCd == "") {
	 			//필수선택 항목입니다.[ IPD 용량 ]
	 			callMsgBox('','W', '필수선택 항목입니다.[ IPD 용량 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].rtfId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].rtfcMdulQutyVal == "") {
	 			 //필수선택 항목입니다.[ 정류모듈 수량 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 정류모듈 수량 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].rtfId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].insVcurRVal == "") {
	 			 //필수선택 항목입니다.[ 입력전류 R ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 입력전류 R ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].rtfId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].insVcurSVal == "") {
	 			 //필수선택 항목입니다.[ 입력전류 S ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 입력전류 S ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].rtfId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].insVcurTVal == "") {
	 			 //필수선택 항목입니다.[ 입력전류 T ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 입력전류 T ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].rtfId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].prtVoltVal == "") {
	 			 //필수선택 항목입니다.[ 출력 전압 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 출력 전압 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].rtfId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].prtVcurVal == "") {
	 			 //필수선택 항목입니다.[ 출력 전류 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 출력 전류 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].rtfId = "";
	    		}
	 			return;
	 		 ...
	 		 callMsgBox('','C', configMsgArray['saveConfirm'], function(msgId, msgRst){
				       //저장한다고 하였을 경우
			        if (msgRst == 'Y') {
			        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/mergeSbeqpRtfInspItmMgmt', dataObj[0], 'POST', 'sbeqpReg');
			        }else{
			        	if(chkNew == "Y"){//+-수정건
			    			dataObj[0].rtfId = "";
			    		}
	...
	function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'dupChkReg'){	// 등록
    		var result = response.sbeqpRtfInspItmDupYn;
    		if(result) {
    			callMsgBox('','i', '동일한 점검일자에 이미 저장된 이력이 있습니다.', function(msgId, msgRst){});
    			var dataObj = $('#'+gridId).alopexGrid("dataGet", {"inspStdDate":response.sbeqpRtfInspItmDupYn});
    	...
    	if(flag == 'search'){	// 상세 View
    		...
    		$('#sbeqpMgmtRtfInspItmForm').setData(result);
    		var param =  $("#sbeqpMgmtRtfInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpRtfInspItmMgmt', param, 'GET', 'gridSearch');
    	}
    	if(flag == 'gridSearch'){	// 점검항목 리스트
    		...
    		var result = response.sbeqpRtfInspItmMgmtList;
//    		addData  = { "rtfId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "ipdCapaValStatCd" : "", "rtfcMdulQutyVal" : "0", "insVcurRVal" : "0", "insVcurSVal" : "0", "insVcurTVal" : "0", "prtVoltVal" : "0", "prtVcurVal" : "0"};
//    		result.unshift(addData);
	       	$('#'+gridId).alopexGrid('dataSet', response.sbeqpRtfInspItmMgmtList);
