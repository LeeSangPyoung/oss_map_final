var main = $a.page(function() {
	...
	var today = new Date();
	var addData  = { "exstrId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "opStatCd" : ""};
	var acceptDate = null;
  ...
  //Grid 초기화
    		...
    		renderMapping: {
    			"date" :  {
					renderer: function(value, data, render, mapping) {
						var $x
						if(data.exstrId == null || data.exstrId == "" || data.exstrId == undefined) {//+-수정건
				...
				"bigo" :  {
					renderer: function(value, data, render, mapping) {
						...
						if (value == undefined || value == "undefined") {
							if(data.exstrId == '' || data.exstrId == null) {
    ...
    function datePicker(gridId, cellId, rowIndex, keyValue){
    	$('#' + cellId + '').showDatePicker( function (date, dateStr ) {
    		...
    		$('#' + gridId + '').alopexGrid("cellEdit", insertDate, {exstrId : ""}, keyValue);
		});
    }
    ...
    function setEventListener() {
    	$("#btnRowAdd").on('click', function(e) {//+-수정건
    		...
    		if(data == null || data == "" || data == undefined){
	    		...
	    		if(data[0].exstrId == null || data[0].exstrId == ""){
		        }else{
		        	$('#'+gridId).alopexGrid('dataAdd', inputData, {_index:{row:0}});
		        	$('#'+gridId).alopexGrid("startEdit");
		        	$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$("#btnRowDel").on('click', function(e) {//+-수정건
      		...
      		if(data == null || data == "" || data == undefined){
		        ...
		        if(data[0].exstrId == null || data[0].exstrId == "" || data[0].exstrId == undefined){
		        	$('#'+gridId).alopexGrid('dataDelete',{_index:{row:0}});
		        	if(chk[0].exstrId =="" || chk[0].exstrId == null || chk[0].exstrId == undefined){
		        		$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$('#'+gridId).on('click', '.bodycell', function(e) {
    		...
    		if (dataObjFoucs[0]._key == "inspStdDate" && (dataObjFoucs[0].exstrId == "" || dataObjFoucs[0].exstrId == null || dataObjFoucs[0].exstrId == undefined) ){
        	...
        	try {
        		...
        		if(dataObj[0]._key != "bigoCol") {
        			if (dataObj[0]._key == "inspStdDate" && dataObj[0]._state.focused && (dataObj[0].exstrId == "" || dataObj[0].exstrId == null || dataObj[0].exstrId == undefined)) {
            			datePicker(gridId, dataObj[0]._index.grid + "-" + dataObj[0]._index.row + "-" + dataObj[0]._index.column, dataObj[0]._index.row, dataObj[0]._key);
                    		...
                    		//+-수정건
                    		if (dataObj[0].exstrId == null || dataObj[0].exstrId == "") {
            			...
            			//+-수정건
            			if(dataObj[0]._state.focused && (dataObj[0].exstrId == "" || dataObj[0].exstrId == null || dataObj[0].exstrId == undefined) && acceptDate != dataObj[0].inspStdDate) {
                			dataObj[0].exstrId = $('#exstrId').val();
                			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpExstrInspItmDupChk',dataObj[0], 'GET', 'dupChk');
                			dataObj[0].exstrId = "";
            			}
            		}
        		}
    	...
    	$('#'+gridId).on('click', "[id=btnDel]", function(e){
    		...
    		callMsgBox('','C', "삭제하시겠습니까?", function(msgId, msgRst){
			       //삭제한다고 하였을 경우
		        if (msgRst == 'Y') {
		        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/deleteSbeqpExstrInspItmInf', dataObj[0], 'POST', 'sbeqpDel');
		        }
	     	});
    	});
  		...
  		$('#'+gridId).on('click', "[id='btnReg']", function(e){
  			...
  			var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
			dataObj[0].exstrId = $('#exstrId').val();
			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpExstrInspItmDupChk',dataObj[0], 'GET', 'dupChkReg');
			dataObj[0].exstrId = "";//+-수정건
		});
  		$('#'+gridId).on('click', "[id='btnMod']", function(e){
  			$('#'+gridId).alopexGrid('saveEdit');
  			sbeqpExstrInspItmReg();
  		});

  		$('#btnSearch').on('click', function(e) {
    		var param =  $("#sbeqpMgmtExstrInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpExstrInspItmMgmt', param, 'GET', 'gridSearch');
  		});
	...
	function sbeqpExstrInspItmReg() {
		var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
		var chkNew = "N";//+-수정건
		var userId;
		if(dataObj[0].exstrId == "" || dataObj[0].exstrId == null || dataObj[0].exstrId == undefined){
			chkNew = "Y";
		}
		...
		dataObj[0].lastChgUserId = userId;
	 		 ...
	 		 callMsgBox('','C', configMsgArray['saveConfirm'], function(msgId, msgRst){
				       //저장한다고 하였을 경우
			        if (msgRst == 'Y') {
			        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/mergeSbeqpExstrInspItmMgmt', dataObj[0], 'POST', 'sbeqpReg');
			        }else{
			        	if(chkNew == "Y"){//+-수정건
			    			dataObj[0].exstrId = "";
			    		}
	...
	function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'dupChkReg'){	// 등록
    		var result = response.sbeqpExstrInspItmDupYn;
    		if(result) {
    			callMsgBox('','i', '동일한 점검일자에 이미 저장된 이력이 있습니다.', function(msgId, msgRst){});
    			var dataObj = $('#'+gridId).alopexGrid("dataGet", {"inspStdDate":response.sbeqpExstrInspItmDupYn});
    	...
    	if(flag == 'search'){	// 상세 View
    		...
    		$('#sbeqpMgmtExstrInspItmForm').setData(result);
    		var param =  $("#sbeqpMgmtExstrInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpExstrInspItmMgmt', param, 'GET', 'gridSearch');
    	}
    	if(flag == 'gridSearch'){	// 점검항목 리스트
    		...
    		var result = response.sbeqpExstrInspItmMgmtList;
//    		addData  = { "exstrId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "opStatCd" : ""};
//    		result.unshift(addData);
	       	$('#'+gridId).alopexGrid('dataSet', response.sbeqpExstrInspItmMgmtList);
//	       	var addCount = 0;
