var main = $a.page(function() {
	...
	var sbeqpId = null;
	var today = new Date();
	var addData  = { "contrId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "insVoltRVal" : "0", "insVcurSVal" : "0", "prtVoltVal" : "0", "prtVcurVal" : "0"};
	var acceptDate = null;
    		...
    		renderMapping: {
    			"date" :  {
					renderer: function(value, data, render, mapping) {
						var $x
						if(data.contrId == null || data.contrId == "" || data.contrId == undefined) {//+-수정건
				...
				"bigo" :  {
					renderer: function(value, data, render, mapping) {
						...
						if (value == undefined || value == "undefined") {
							if(data.contrId == '' || data.contrId == null) {
...
//					}
        	...
        	columnMapping: [{
				...
				key : 'insVoltRVal', align:'center', title : '입력전압', width: '60px', editable: { type: 'maskedInput' }
			}, {/* 입력전류              */
				key : 'insVcurSVal', align:'center', title : '입력전류', width: '60px', editable: { type: 'maskedInput' }
			}, {/* 출력전압              */
    ...
    function datePicker(gridId, cellId, rowIndex, keyValue){
    	$('#' + cellId + '').showDatePicker( function (date, dateStr ) {
    		...
    		$('#' + gridId + '').alopexGrid("cellEdit", insertDate, {contrId : ""}, keyValue);
		});
    }
    ...
    function setEventListener() {
    	$("#btnRowAdd").on('click', function(e) {//+-수정건
    		...
    		if(data == null || data == "" || data == undefined){
	    		...
	    		if(data[0].contrId == null || data[0].contrId == ""){
		        }else{
		        	$('#'+gridId).alopexGrid('dataAdd', inputData, {_index:{row:0}});
		        	$('#'+gridId).alopexGrid("startEdit");
		        	$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$("#btnRowDel").on('click', function(e) {//+-수정건
      		...
      		if(data == null || data == "" || data == undefined){
		        ...
		        if(data[0].contrId == null || data[0].contrId == "" || data[0].contrId == undefined){
		        	$('#'+gridId).alopexGrid('dataDelete',{_index:{row:0}});
		        	if(chk[0].contrId =="" || chk[0].contrId == null || chk[0].contrId == undefined){
		        		$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$('#'+gridId).on('click', '.bodycell', function(e) {
    		...
    		if (dataObjFoucs[0]._key == "inspStdDate" && (dataObjFoucs[0].contrId == "" || dataObjFoucs[0].contrId == null || dataObjFoucs[0].contrId == undefined) ){
        	...
        	try {
        		...
        		if(dataObj[0]._key != "bigoCol") {
        			if (dataObj[0]._key == "inspStdDate" && dataObj[0]._state.focused && (dataObj[0].contrId == "" || dataObj[0].contrId == null || dataObj[0].contrId == undefined)) {
            			datePicker(gridId, dataObj[0]._index.grid + "-" + dataObj[0]._index.row + "-" + dataObj[0]._index.column, dataObj[0]._index.row, dataObj[0]._key);
                    		...
                    		//+-수정건
                    		if (dataObj[0].contrId == null || dataObj[0].contrId == "") {
            			...
            			//+-수정건
            			if(dataObj[0]._state.focused && (dataObj[0].contrId == "" || dataObj[0].contrId == null || dataObj[0].contrId == undefined) && acceptDate != dataObj[0].inspStdDate) {
                			dataObj[0].contrId = $('#contrId').val();
                			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpContrInspItmDupChk',dataObj[0], 'GET', 'dupChk');
                			dataObj[0].contrId = "";
            			}
            		}
        		}
    	...
    	$('#'+gridId).on('click', "[id=btnDel]", function(e){
    		...
    		callMsgBox('','C', "삭제하시겠습니까?", function(msgId, msgRst){
			       //삭제한다고 하였을 경우
		        if (msgRst == 'Y') {
		        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/deleteSbeqpContrInspItmInf', dataObj[0], 'POST', 'sbeqpDel');
		        }
	     	});
    	});
  		...
  		$('#'+gridId).on('click', "[id='btnReg']", function(e){
  			...
  			var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
			dataObj[0].contrId = $('#contrId').val();
			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpContrInspItmDupChk',dataObj[0], 'GET', 'dupChkReg');
			dataObj[0].contrId = "";//+-수정건

		});
  		$('#'+gridId).on('click', "[id='btnMod']", function(e){
  			$('#'+gridId).alopexGrid('saveEdit');
  			sbeqpContrInspItmReg();
  		});

  		$('#btnSearch').on('click', function(e) {
    		var param =  $("#sbeqpMgmtContrInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpContrInspItmMgmt', param, 'GET', 'gridSearch');
  		});
	...
	function sbeqpContrInspItmReg() {
		var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
		var chkNew = "N";//+-수정건
		var userId;
		if(dataObj[0].contrId == "" || dataObj[0].contrId == null || dataObj[0].contrId == undefined){
			chkNew = "Y";
		}
		...
		dataObj[0].lastChgUserId = userId;
 		 ...
 		 if (dataObj[0].insVoltRVal == "") {
 			//필수선택 항목입니다.[ 입력전압 ]
 			callMsgBox('','W', '필수선택 항목입니다.[ 입력전압 ]', function(msgId, msgRst){});
 			if(chkNew == "Y"){//+-수정건
    			dataObj[0].contrId = "";
    		}
 			return;
 		...
 		if (dataObj[0].insVcurSVal == "") {
 			//필수선택 항목입니다.[ 입력전류 ]
 			callMsgBox('','W', '필수선택 항목입니다.[ 입력전류 ]', function(msgId, msgRst){});
 			if(chkNew == "Y"){//+-수정건
    			dataObj[0].contrId = "";
 			}
 			return;
 		...
 		if (dataObj[0].prtVoltVal == "") {
 			//필수선택 항목입니다.[ 출력전압 ]
 			callMsgBox('','W', '필수선택 항목입니다.[ 출력전압 ]', function(msgId, msgRst){});
 			if(chkNew == "Y"){//+-수정건
    			dataObj[0].contrId = "";
    		}
 			return;
 		...
 		if (dataObj[0].prtVcurVal == "") {
 			//필수선택 항목입니다.[ 출력전류 ]
 			callMsgBox('','W', '필수선택 항목입니다.[ 출력전류 ]', function(msgId, msgRst){});
 			if(chkNew == "Y"){//+-수정건
    			dataObj[0].contrId = "";
    		}
 			return;
 		 ...
 		 callMsgBox('','C', configMsgArray['saveConfirm'], function(msgId, msgRst){
			       //저장한다고 하였을 경우
		        if (msgRst == 'Y') {
		        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/mergeSbeqpContrInspItmMgmt', dataObj[0], 'POST', 'sbeqpReg');
		        }else{
		        	if(chkNew == "Y"){//+-수정건
		    			dataObj[0].contrId = "";
		    		}
	...
	function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'dupChkReg'){	// 등록
    		var result = response.sbeqpContrInspItmDupYn;
    		if(result) {
    			callMsgBox('','i', '동일한 점검일자에 이미 저장된 이력이 있습니다.', function(msgId, msgRst){});
    			var dataObj = $('#'+gridId).alopexGrid("dataGet", {"inspStdDate":response.sbeqpContrInspItmDupYn});
    	...
    	if(flag == 'search'){	// 상세 View
    		...
    		$('#sbeqpMgmtContrInspItmForm').setData(result);
    		var param =  $("#sbeqpMgmtContrInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpContrInspItmMgmt', param, 'GET', 'gridSearch');
    	}
    	if(flag == 'gridSearch'){	// 점검항목 리스트
    		...
    		var result = response.sbeqpContrInspItmMgmtList;
//    		addData  = { "contrId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "insVoltRVal" : "0", "insVcurSVal" : "0", "prtVoltVal" : "0", "prtVcurVal" : "0"};
//    		result.unshift(addData);
	       	$('#'+gridId).alopexGrid('dataSet', response.sbeqpContrInspItmMgmtList);
//	       	var addCount = 0;
