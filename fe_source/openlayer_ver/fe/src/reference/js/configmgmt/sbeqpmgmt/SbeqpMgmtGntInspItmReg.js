var main = $a.page(function() {
	...
	var today = new Date();
	var addData  = { "gntId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "fuelTankCapaVal" : "0", "staExusBatryStatCd" : "", "engolStatCd" : "", "colntStatCd" : "", "wuhtrStatCd" : "", "exstrnStatCd" : "", "ctplStatCd" : "", "tlrnLoadStatCd" : "", "tlrnRsltStatCd" : "", "atsSwchgTestStatCd" : ""};
	var acceptDate = null;
    		...
    		renderMapping: {
    			"date" :  {
					renderer: function(value, data, render, mapping) {
						var $x
						if(data.gntId == null || data.gntId == "" || data.gntId == undefined) {//+-수정건
...
//					}
    		...
    		headerGroup : [
	               { fromIndex : 2, toIndex : 2, title:'연료탱크'},
	               { fromIndex : 3, toIndex : 3, title:'시동용축전지'},
	               { fromIndex : 9, toIndex : 11, title:'시운전'}
	        ],
        	columnMapping: [{
				...
				key : 'fuelTankCapaVal', align:'center', title : '보유량', width: '80px', editable: {type:'maskedInput'}
			}, {/* 운용상태 		 */
				key : 'staExusBatryStatCd', align:'center', title : '운용상태', width: '100px',
				render : {
					type : 'string',
				...
				key : 'engolStatCd', align:'center', title : "<div style='line-height:150%'>엔진오일 점검<br>(상태, 누유 등)</div>", width: '100px',
				render : {
					type : 'string',
					rule : function(value, data) {
						var render_data = [];
				...
				key : 'colntStatCd', align:'center', title : "<div style='line-height:150%'>냉각수 점검<br>(상태, 누수 등)</div>", width: '100px',
				render : {
					type : 'string',
					rule : function(value, data) {
						var render_data = [];
				...
				key : 'wuhtrStatCd', align:'center', title : '예열 히터 상태 ', width: '100px',
				render : {
					type : 'string',
					rule : function(value, data) {
						var render_data = [];
				...
				key : 'exstrnStatCd', align:'center', title : "<div style='line-height:150%'>배과, 호스, 흡/배기<br> 상태</div>", width: '120px',
				render : {
					type : 'string',
					rule : function(value, data) {
						var render_data = [];
				...
				key : 'ctplStatCd', align:'center', title : "<div style='line-height:150%'>제어판 상태<br>(표시램프, 레버, 계기등)</div>", width: '150px',
				render : {
					type : 'string',
					rule : function(value, data) {
						var render_data = [];
				...
				key : 'tlrnLoadStatCd', align:'center', title : '부하/무부하', width: '100px',
				render : {
					type : 'string',
					rule : function(value, data) {
						var render_data = [];
				...
				key : 'tlrnRsltStatCd', align:'center', title : '시운전결과', width: '100px',
				render : {
					type : 'string',
					rule : function(value, data) {
						var render_data = [];
				...
				key : 'atsSwchgTestStatCd', align:'center', title : 'ATS절체 시험결과', width: '120px',
				render : {
					type : 'string',
					rule : function(value, data) {
						var render_data = [];
    ...
    function datePicker(gridId, cellId, rowIndex, keyValue){
    	$('#' + cellId + '').showDatePicker( function (date, dateStr ) {
    		...
    		$('#' + gridId + '').alopexGrid("cellEdit", insertDate, {gntId : ""}, keyValue);
		});
    }
    ...
    function setEventListener() {
    	$("#btnRowAdd").on('click', function(e) {//+-수정건
    		...
    		if(data == null || data == "" || data == undefined){
	    		...
	    		if(data[0].gntId == null || data[0].gntId == ""){
		        }else{
		        	$('#'+gridId).alopexGrid('dataAdd', inputData, {_index:{row:0}});
		        	$('#'+gridId).alopexGrid("startEdit");
		        	$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$("#btnRowDel").on('click', function(e) {//+-수정건
      		...
      		if(data == null || data == "" || data == undefined){
		        ...
		        if(data[0].gntId == null || data[0].gntId == "" || data[0].gntId == undefined){
		        	$('#'+gridId).alopexGrid('dataDelete',{_index:{row:0}});
		        	if(chk[0].gntId =="" || chk[0].gntId == null || chk[0].gntId == undefined){
		        		$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$('#'+gridId).on('click', '.bodycell', function(e) {
    		...
    		if (dataObjFoucs[0]._key == "inspStdDate" && (dataObjFoucs[0].gntId == "" || dataObjFoucs[0].gntId == null || dataObjFoucs[0].gntId == undefined) ){
        	...
        	try {
        		...
        		if(dataObj[0]._key != "bigoCol") {
        			if (dataObj[0]._key == "inspStdDate" && dataObj[0]._state.focused && (dataObj[0].gntId == "" || dataObj[0].gntId == null || dataObj[0].gntId == undefined)) {
            			datePicker(gridId, dataObj[0]._index.grid + "-" + dataObj[0]._index.row + "-" + dataObj[0]._index.column, dataObj[0]._index.row, dataObj[0]._key);
                    		...
                    		//+-수정건
                    		if (dataObj[0].gntId == null || dataObj[0].gntId == "") {
            			...
            			//+-수정건
            			if(dataObj[0]._state.focused && (dataObj[0].gntId == "" || dataObj[0].gntId == null || dataObj[0].gntId == undefined) && acceptDate != dataObj[0].inspStdDate) {
                			dataObj[0].gntId = $('#gntId').val();
                			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpGntInspItmDupChk',dataObj[0], 'GET', 'dupChk');
                			dataObj[0].gntId = "";
            			}
            		}
        		}
    	...
    	$('#'+gridId).on('click', "[id=btnDel]", function(e){
    		...
    		callMsgBox('','C', "삭제하시겠습니까?", function(msgId, msgRst){
			       //삭제한다고 하였을 경우
		        if (msgRst == 'Y') {
		        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/deleteSbeqpGntInspItmInf', dataObj[0], 'POST', 'sbeqpDel');
		        }
	     	});
    	});
  		...
  		$('#'+gridId).on('click', "[id='btnReg']", function(e){
  			...
  			var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
			dataObj[0].gntId = $('#gntId').val();
			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpGntInspItmDupChk',dataObj[0], 'GET', 'dupChkReg');
			dataObj[0].gntId = "";//+-수정건
		});
  		...
  		$('#btnSearch').on('click', function(e) {
    		var param =  $("#sbeqpMgmtGntInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpGntInspItmMgmt', param, 'GET', 'gridSearch');
  		});
	...
	function sbeqpGntInspItmReg() {
		var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
		var chkNew = "N";//+-수정건
		var userId;
		if(dataObj[0].gntId == "" || dataObj[0].gntId == null || dataObj[0].gntId == undefined){
			chkNew = "Y";
		}
		...
		dataObj[0].lastChgUserId = userId;
	 		 ...
	 		 if (dataObj[0].fuelTankCapaVal == "") {
	 			//필수선택 항목입니다.[ 연료탱크 보유량 ]
	 			callMsgBox('','W', '필수선택 항목입니다.[ 연료탱크 보유량 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].gntId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].staExusBatryStatCd == "") {
	 			 //필수선택 항목입니다.[ 시동용축전지 운용상태 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 시동용축전지 운용상태 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].gntId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].engolStatCd == "") {
	 			 //필수선택 항목입니다.[ 엔진오일 점검 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 엔진오일 점검 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].gntId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].colntStatCd == "") {
	 			 //필수선택 항목입니다.[ 냉각수 점검 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 냉각수 점검 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].gntId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].wuhtrStatCd == "") {
	 			 //필수선택 항목입니다.[ 예열 히터 상태 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 예열 히터 상태 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].gntId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].exstrnStatCd == "") {
	 			 //필수선택 항목입니다.[ 배과, 호스, 흡/배기 상태 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 배과, 호스, 흡/배기 상태 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].gntId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].ctplStatCd == "") {
	 			 //필수선택 항목입니다.[ 제어판 상태 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 제어판 상태 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].gntId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].tlrnLoadStatCd == "") {
	 			 //필수선택 항목입니다.[ 부하/무부하 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 부하/무부하 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].gntId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].tlrnRsltStatCd == "") {
	 			 //필수선택 항목입니다.[ 시운전 상태 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 시운전 상태 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].gntId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].atsSwchgTestStatCd == "") {
	 			 //필수선택 항목입니다.[ ATS절체 시험결과 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ ATS절체 시험결과 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].gntId = "";
	    		}
	 			return;
	 		 ...
	 		 callMsgBox('','C', configMsgArray['saveConfirm'], function(msgId, msgRst){
				       //저장한다고 하였을 경우
			        if (msgRst == 'Y') {
			        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/mergeSbeqpGntInspItmMgmt', dataObj[0], 'POST', 'sbeqpReg');
			        }else{
			        	if(chkNew == "Y"){//+-수정건
			    			dataObj[0].gntId = "";
			    		}
	...
	function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'dupChkReg'){	// 등록
    		var result = response.sbeqpGntInspItmDupYn;
    		if(result) {
    			callMsgBox('','i', '동일한 점검일자에 이미 저장된 이력이 있습니다.', function(msgId, msgRst){});
    			var dataObj = $('#'+gridId).alopexGrid("dataGet", {"inspStdDate":response.sbeqpGntInspItmDupYn});
    	...
    	if(flag == 'search'){	// 상세 View
    		...
    		$('#sbeqpMgmtGntInspItmForm').setData(result);
    		var param =  $("#sbeqpMgmtGntInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpGntInspItmMgmt', param, 'GET', 'gridSearch');
    	}
    	if(flag == 'gridSearch'){	// 점검항목 리스트
    		...
    		var result = response.sbeqpGntInspItmMgmtList;
//    		addData  = { "gntId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "fuelTankCapaVal" : "0", "staExusBatryStatCd" : "", "engolStatCd" : "", "colntStatCd" : "", "wuhtrStatCd" : "", "exstrnStatCd" : "", "ctplStatCd" : "", "tlrnLoadStatCd" : "", "tlrnRsltStatCd" : "", "atsSwchgTestStatCd" : ""};
//    		result.unshift(addData);
	       	$('#'+gridId).alopexGrid('dataSet', response.sbeqpGntInspItmMgmtList);
