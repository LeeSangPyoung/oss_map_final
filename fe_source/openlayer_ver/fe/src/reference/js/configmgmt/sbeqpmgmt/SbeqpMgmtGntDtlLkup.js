/**
 * EpwrStcMgmtGntRsltReg.js
 *
 * @author Administrator
 * @date 2018. 02. 28.
 * @version 1.0
 */
var sbeqpGntDtl = $a.page(function() {
    //초기 진입점
	var paramData = null;
	var sbeqpId = null;
    //param에 이전 페이지에서 넘겨준 파라미터가 json형태로 저장 되어 있음.
    this.init = function(id, param) {
    	window.resizeTo(900,820);
    	$a.maskedinput($('#gntPartMdlMnftYmReg')[0], '0000-00-00');
    	$a.maskedinput($('#atsPartMdlMnftYmReg')[0], '0000-00-00');
    ...
    function setRegDataSet(data) {
    	$(':input','#sbeqpMgmtGntDtlLkupArea').val('');
    }
    ...
    function setEventListener() {
    	...
    	//장비수정
			...
			var param =  $("#sbeqpMgmtGntDtlLkupArea").getData();
			param.regYn = "Y";
			...
			var param =  $("#sbeqpMgmtGntDtlLkupArea").getData();
			param.regYn = "Y";
			$a.navigate('/tango-transmission-web/configmgmt/sbeqpmgmt/SbeqpMgmtGntInspItmReg.do',param);
        });
    ...
    function successCallback(response, status, jqxhr, flag){
    	if(flag == 'search'){
    		...
    		$('#sbeqpMgmtGntDtlLkupArea').setData(result);
    	}
    }
    //request 실패시.
