var main = $a.page(function() {
	...
	var today = new Date();
	var addData  = { "rtfMdulId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "fanStatCd" : "", "prtVoltVal" : "0", "prtVcurVal" : "0"};
	var acceptDate = null;
    		...
    		renderMapping: {
    			"date" :  {
					renderer: function(value, data, render, mapping) {
						var $x
						if(data.rtfMdulId == null || data.rtfMdulId == "" || data.rtfMdulId == undefined) {//+-수정건
				...
				"bigo" :  {
					renderer: function(value, data, render, mapping) {
						...
						if (value == undefined || value == "undefined") {
							if(data.rtfMdulId == '' || data.rtfMdulId == null) {
...
//					}
    		...
    		headerGroup : [
	               { fromIndex : 2, toIndex : 3, title:'출력전압/전류'}
	        ],
        	columnMapping: [{
				...
				key : 'fanStatCd', align:'center', title : 'Fan상태', width: '70px',
				render : {
      	    		type : 'string',
    ...
    function datePicker(gridId, cellId, rowIndex, keyValue){
    	$('#' + cellId + '').showDatePicker( function (date, dateStr ) {
    		...
    		$('#' + gridId + '').alopexGrid("cellEdit", insertDate, {rtfMdulId : ""}, keyValue);
		});
    }
    ...
    function setEventListener() {
    	$("#btnRowAdd").on('click', function(e) {//+-수정건
    		...
    		if(data == null || data == "" || data == undefined){
	    		...
	    		if(data[0].rtfMdulId == null || data[0].rtfMdulId == ""){
		        }else{
		        	$('#'+gridId).alopexGrid('dataAdd', inputData, {_index:{row:0}});
		        	$('#'+gridId).alopexGrid("startEdit");
		        	$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$("#btnRowDel").on('click', function(e) {//+-수정건
      		...
      		if(data == null || data == "" || data == undefined){
		        ...
		        if(data[0].rtfMdulId == null || data[0].rtfMdulId == "" || data[0].rtfMdulId == undefined){
		        	$('#'+gridId).alopexGrid('dataDelete',{_index:{row:0}});
		        	if(chk[0].rtfMdulId =="" || chk[0].rtfMdulId == null || chk[0].rtfMdulId == undefined){
		        		$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$('#'+gridId).on('click', '.bodycell', function(e) {
    		...
    		if (dataObjFoucs[0]._key == "inspStdDate" && (dataObjFoucs[0].rtfMdulId == "" || dataObjFoucs[0].rtfMdulId == null || dataObjFoucs[0].rtfMdulId == undefined) ){
        	...
        	try {
        		...
        		if(dataObj[0]._key != "bigoCol") {
        			if (dataObj[0]._key == "inspStdDate" && dataObj[0]._state.focused && (dataObj[0].rtfMdulId == "" || dataObj[0].rtfMdulId == null || dataObj[0].rtfMdulId == undefined)) {
            			datePicker(gridId, dataObj[0]._index.grid + "-" + dataObj[0]._index.row + "-" + dataObj[0]._index.column, dataObj[0]._index.row, dataObj[0]._key);
                    		...
                    		//+-수정건
                    		if (dataObj[0].rtfMdulId == null || dataObj[0].rtfMdulId == "") {
            			...
            			//+-수정건
            			if(dataObj[0]._state.focused && (dataObj[0].rtfMdulId == "" || dataObj[0].rtfMdulId == null || dataObj[0].rtfMdulId == undefined) && acceptDate != dataObj[0].inspStdDate) {
                			dataObj[0].rtfMdulId = $('#rtfMdulId').val();
                			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpRtfMdulInspItmDupChk',dataObj[0], 'GET', 'dupChk');
                			dataObj[0].rtfMdulId = "";
            			}
            		}
        		}
    	...
    	$('#'+gridId).on('click', "[id=btnDel]", function(e){
    		...
    		callMsgBox('','C', "삭제하시겠습니까?", function(msgId, msgRst){
			       //삭제한다고 하였을 경우
		        if (msgRst == 'Y') {
		        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/deleteSbeqpRtfMdulInspItmInf', dataObj[0], 'POST', 'sbeqpDel');
		        }
	     	});
  		...
  		$('#'+gridId).on('click', "[id='btnReg']", function(e){
  			...
  			var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
			dataObj[0].rtfMdulId = $('#rtfMdulId').val();
			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpRtfMdulInspItmDupChk',dataObj[0], 'GET', 'dupChkReg');
			dataObj[0].rtfMdulId = "";//+-수정건
		});
  		$('#'+gridId).on('click', "[id='btnMod']", function(e){
  			$('#'+gridId).alopexGrid('saveEdit');
  			sbeqpRtfMdulInspItmReg();
  		});

  		$('#btnSearch').on('click', function(e) {
    		var param =  $("#sbeqpMgmtRtfMdulInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpRtfMdulInspItmMgmt', param, 'GET', 'gridSearch');
  		});
	...
	function sbeqpRtfMdulInspItmReg() {
		var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
		var chkNew = "N";//+-수정건
		var userId;
		if(dataObj[0].rtfMdulId == "" || dataObj[0].rtfMdulId == null || dataObj[0].rtfMdulId == undefined){
			chkNew = "Y";
		}
		...
		dataObj[0].lastChgUserId = userId;
	 		 ...
	 		 if (dataObj[0].fanStatCd == "") {
		 			//필수선택 항목입니다.[ Fan 상태 ]
		 			callMsgBox('','W', '필수선택 항목입니다.[ Fan 상태 ]', function(msgId, msgRst){});
		 			if(chkNew == "Y"){//+-수정건
		    			dataObj[0].rtfMdulId = "";
		    		}
		 			return;
	 		 ...
	 		 callMsgBox('','C', configMsgArray['saveConfirm'], function(msgId, msgRst){
				       //저장한다고 하였을 경우
			        if (msgRst == 'Y') {
			        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/mergeSbeqpRtfMdulInspItmMgmt', dataObj[0], 'POST', 'sbeqpReg');
			        }else{
			        	if(chkNew == "Y"){//+-수정건
			    			dataObj[0].rtfMdulId = "";
			    		}
	...
	function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'dupChkReg'){	// 등록
    		var result = response.sbeqpRtfMdulInspItmDupYn;
    		if(result) {
    			callMsgBox('','i', '동일한 점검일자에 이미 저장된 이력이 있습니다.', function(msgId, msgRst){});
    			var dataObj = $('#'+gridId).alopexGrid("dataGet", {"inspStdDate":response.sbeqpRtfMdulInspItmDupYn});
    	...
    	if(flag == 'search'){	// 상세 View
    		...
    		$('#sbeqpMgmtRtfMdulInspItmForm').setData(result);
    		var param =  $("#sbeqpMgmtRtfMdulInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpRtfMdulInspItmMgmt', param, 'GET', 'gridSearch');
    	}
    	if(flag == 'gridSearch'){	// 점검항목 리스트
    		...
    		var result = response.sbeqpRtfMdulInspItmMgmtList;
//    		addData  = { "rtfMdulId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "fanStatCd" : "", "prtVoltVal" : "0", "prtVcurVal" : "0"};
//    		result.unshift(addData);
	       	$('#'+gridId).alopexGrid('dataSet', response.sbeqpRtfMdulInspItmMgmtList);
