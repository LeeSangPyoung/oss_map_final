var main = $a.page(function() {
	...
	var staDay = null;
	var sbeqpId = null;
	var capaCdData = [{text:'선택', value:''},{text:'양호', value:'G'}, {text:'불량', value:'B'}];
	var today = new Date();
	var addData  = { "tvssId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "opStatCd" : ""};
	var acceptDate = null;
  ...
  //Grid 초기화
    function initGrid() {
        ...
        $('#'+gridId).alopexGrid({
    		...
    		renderMapping: {
    			"date" :  {
					renderer: function(value, data, render, mapping) {
						var $x
						if(data.tvssId == null || data.tvssId == "" || data.tvssId == undefined) {//+-수정건
				...
				"bigo" :  {
					renderer: function(value, data, render, mapping) {
						...
						if (value == undefined || value == "undefined") {
							if(data.tvssId == '' || data.tvssId == null) {
        	...
        	columnMapping: [{
    			...
    			key : 'inspStdDate', align:'center', title : '날짜', width: '120px', render : { type: 'date' }
			}, {/* 운용상태  */
				key : 'opStatCd', align:'center', title : '운용상태', width: '60px',
				render : {
      	    		type : 'string',
				...
				key : 'bigoCol', align:'center', title : '비고', width: '400px', render : { type: 'bigo' }
			}]
        });
    ...
    function datePicker(gridId, cellId, rowIndex, keyValue){
    	$('#' + cellId + '').showDatePicker( function (date, dateStr ) {
    		...
    		$('#' + gridId + '').alopexGrid("cellEdit", insertDate, {tvssId : ""}, keyValue);
		});
    }
    ...
    function setEventListener() {
    	$("#btnRowAdd").on('click', function(e) {//+-수정건
    		...
    		if(data == null || data == "" || data == undefined){
	    		...
	    		if(data[0].tvssId == null || data[0].tvssId == ""){
		        }else{
		        	$('#'+gridId).alopexGrid('dataAdd', inputData, {_index:{row:0}});
		        	$('#'+gridId).alopexGrid("startEdit");
		        	$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$("#btnRowDel").on('click', function(e) {//+-수정건
      		...
      		if(data == null || data == "" || data == undefined){
		        ...
		        if(data[0].tvssId == null || data[0].tvssId == "" || data[0].tvssId == undefined){
		        	$('#'+gridId).alopexGrid('dataDelete',{_index:{row:0}});
		        	if(chk[0].tvssId =="" || chk[0].tvssId == null || chk[0].tvssId == undefined){
		        		$('#'+gridId).alopexGrid('cellEdit',"", {_state:{selected:true}}, 'bigoCol');
    	...
    	$('#'+gridId).on('click', '.bodycell', function(e) {
    		...
    		if (dataObjFoucs[0]._key == "inspStdDate" && (dataObjFoucs[0].tvssId == "" || dataObjFoucs[0].tvssId == null || dataObjFoucs[0].tvssId == undefined) ){
        	...
        	try {
        		...
        		if(dataObj[0]._key != "bigoCol") {
        			if (dataObj[0]._key == "inspStdDate" && dataObj[0]._state.focused && (dataObj[0].tvssId == "" || dataObj[0].tvssId == null || dataObj[0].tvssId == undefined)) {
            			datePicker(gridId, dataObj[0]._index.grid + "-" + dataObj[0]._index.row + "-" + dataObj[0]._index.column, dataObj[0]._index.row, dataObj[0]._key);
                    		...
                    		//+-수정건
                    		if (dataObj[0].tvssId == null || dataObj[0].tvssId == "") {
            			...
            			//+-수정건
            			if(dataObj[0]._state.focused && (dataObj[0].tvssId == "" || dataObj[0].tvssId == null || dataObj[0].tvssId == undefined) && acceptDate != dataObj[0].inspStdDate) {
                			dataObj[0].tvssId = $('#tvssId').val();
                			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpTvssInspItmDupChk',dataObj[0], 'GET', 'dupChk');
                			dataObj[0].tvssId = "";
            			}
            		}
        		}
    	...
    	$('#'+gridId).on('click', "[id=btnDel]", function(e){
    		...
    		callMsgBox('','C', "삭제하시겠습니까?", function(msgId, msgRst){
			       //삭제한다고 하였을 경우
		        if (msgRst == 'Y') {
		        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/deleteSbeqpTvssInspItmInf', dataObj[0], 'POST', 'sbeqpDel');
		        }
	     	});
    	});
  		...
  		$('#'+gridId).on('click', "[id='btnReg']", function(e){
  			...
  			var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
			dataObj[0].tvssId = $('#tvssId').val();
			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpTvssInspItmDupChk',dataObj[0], 'GET', 'dupChkReg');
			dataObj[0].tvssId = "";//+-수정건
		});
  		...
  		$('#btnSearch').on('click', function(e) {
    		var param =  $("#sbeqpMgmtTvssInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpTvssInspItmMgmt', param, 'GET', 'gridSearch');
  		});
	...
	function sbeqpTvssInspItmReg() {
		var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
		var chkNew = "N";//+-수정건
		var userId;
		if(dataObj[0].tvssId == "" || dataObj[0].tvssId == null || dataObj[0].tvssId == undefined){
			chkNew = "Y";
		}
		...
		dataObj[0].lastChgUserId = userId;
	 		 ...
	 		 if (dataObj[0].opStatCd == "") {
	 			//필수선택 항목입니다.[ 운용상태 ]
	 			callMsgBox('','W', '필수선택 항목입니다.[ 운용상태 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].tvssId = "";
	    		}
	 			return;
	 		 ...
	 		 callMsgBox('','C', configMsgArray['saveConfirm'], function(msgId, msgRst){
				       //저장한다고 하였을 경우
			        if (msgRst == 'Y') {
			        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/mergeSbeqpTvssInspItmMgmt', dataObj[0], 'POST', 'sbeqpReg');
			        }else{
			        	if(chkNew == "Y"){//+-수정건
			    			dataObj[0].tvssId = "";
			    		}
	...
	function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'dupChkReg'){	// 등록
    		var result = response.sbeqpTvssInspItmDupYn;
    		if(result) {
    			callMsgBox('','i', '동일한 점검일자에 이미 저장된 이력이 있습니다.', function(msgId, msgRst){});
    			var dataObj = $('#'+gridId).alopexGrid("dataGet", {"inspStdDate":response.sbeqpTvssInspItmDupYn});
    	...
    	if(flag == 'search'){	// 상세 View
    		...
    		$('#sbeqpMgmtTvssInspItmForm').setData(result);
    		var param =  $("#sbeqpMgmtTvssInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpTvssInspItmMgmt', param, 'GET', 'gridSearch');
    	}
    	if(flag == 'gridSearch'){	// 점검항목 리스트
    		...
    		var result = response.sbeqpTvssInspItmMgmtList;
//    		addData  = { "tvssId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "opStatCd" : ""};
//    		result.unshift(addData);
	       	$('#'+gridId).alopexGrid('dataSet', response.sbeqpTvssInspItmMgmtList);
//	       	var addCount = 0;
