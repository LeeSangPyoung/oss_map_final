var main = $a.page(function() {
	...
	var today = new Date();
	var addData  = { "batryId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "ouppInspStatCd" : "", "sfsTmpr" : "0"};
	var acceptDate = null;
...
//					}
        	...
        	columnMapping: [{
				...
				key : 'sfsTmpr', align:'center', title : '표면온도', width: '60px', editable: {type:'maskedInput'}
			}, {/* 외관점검 		 */
				key : 'ouppInspStatCd', align:'center', title : '외관점검', width: '70px',
				render : {
      	    		type : 'string',
				...
				key : 'bigoCol', align:'center', title : '비고', width: '460px', render : { type: 'bigo' }
			}]
        });
    ...
    function setEventListener() {
    	...
    	$('#'+gridId).on('click', '.bodycell', function(e) {
    		...
    		if (dataObjFoucs[0]._key == "inspStdDate" && (dataObjFoucs[0].batryId == "" || dataObjFoucs[0].batryId == null || dataObjFoucs[0].batryId == undefined) ){
        	...
        	try {
        		...
        		if(dataObj[0]._key != "bigoCol") {
        			if (dataObj[0]._key == "inspStdDate" && dataObj[0]._state.focused && (dataObj[0].batryId == "" || dataObj[0].batryId == null || dataObj[0].batryId == undefined)) {
            			...
            			//+-수정건
            			if(dataObj[0]._state.focused && (dataObj[0].batryId == "" || dataObj[0].batryId == null || dataObj[0].batryId == undefined) && acceptDate != dataObj[0].inspStdDate) {
                			dataObj[0].batryId = $('#batryId').val();
                			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpBatryInspItmDupChk',dataObj[0], 'GET', 'dupChk');
                			dataObj[0].batryId = "";
            			}
            		}
        		}
    	...
    	$('#'+gridId).on('click', "[id=btnDel]", function(e){
    		...
    		callMsgBox('','C', "삭제하시겠습니까?", function(msgId, msgRst){
			       //삭제한다고 하였을 경우
		        if (msgRst == 'Y') {
		        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/deleteSbeqpBatryInspItmInf', dataObj[0], 'POST', 'sbeqpDel');
		        }
	     	});
    	});
  		...
  		$('#'+gridId).on('click', "[id='btnReg']", function(e){
  			...
  			var dataObj = $('#'+gridId).alopexGrid('dataGet',{_state:{selected:true}});
			dataObj[0].batryId = $('#batryId').val();
			httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/getSbeqpBatryInspItmDupChk',dataObj[0], 'GET', 'dupChkReg');
			dataObj[0].batryId = "";//+-수정건
		});
  		$('#'+gridId).on('click', "[id='btnMod']", function(e){
  			$('#'+gridId).alopexGrid('saveEdit');
  			sbeqpBatryInspItmReg();
  		});

  		$('#btnSearch').on('click', function(e) {
    		var param =  $("#sbeqpMgmtBatryInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpBatryInspItmMgmt', param, 'GET', 'gridSearch');
  		});
	...
	function sbeqpBatryInspItmReg() {
		...
		dataObj[0].lastChgUserId = userId;
	 		 ...
	 		 if (dataObj[0].sfsTmpr == "") {
	 			//필수선택 항목입니다.[ 표면온도 ]
	 			callMsgBox('','W', '필수선택 항목입니다.[ 표면온도 ]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].batryId = "";
	    		}
	 			return;
	 		 }
	 		 if (dataObj[0].ouppInspStatCd == "") {
	 			 //필수선택 항목입니다.[ 외관점검 ]
	 			 callMsgBox('','W', '필수선택 항목입니다.[ 외관점검]', function(msgId, msgRst){});
	 			if(chkNew == "Y"){//+-수정건
	    			dataObj[0].batryId = "";
	    		}
	 			 return;
	 		 ...
	 		 callMsgBox('','C', configMsgArray['saveConfirm'], function(msgId, msgRst){
				       //저장한다고 하였을 경우
			        if (msgRst == 'Y') {
			        	httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/mergeSbeqpBatryInspItmMgmt', dataObj[0], 'POST', 'sbeqpReg');
			        }else{
			        	if(chkNew == "Y"){//+-수정건
			    			dataObj[0].batryId = "";
			    		}
	...
	function successCallback(response, status, jqxhr, flag){
    	...
    	if(flag == 'dupChkReg'){	// 등록
    		var result = response.sbeqpBatryInspItmDupYn;
    		if(result) {
    			callMsgBox('','i', '동일한 점검일자에 이미 저장된 이력이 있습니다.', function(msgId, msgRst){});
    			var dataObj = $('#'+gridId).alopexGrid("dataGet", {"inspStdDate":response.sbeqpBatryInspItmDupYn});
    	...
    	if(flag == 'search'){	// 상세 View
    		...
    		$('#sbeqpMgmtBatryInspItmForm').setData(result);
    		var param =  $("#sbeqpMgmtBatryInspItmForm").getData();
    		$('#'+gridId).alopexGrid('showProgress');
    		httpRequest('tango-transmission-biz/transmisson/configmgmt/sbeqpmgmt/sbeqpBatryInspItmMgmt', param, 'GET', 'gridSearch');
    	}
    	if(flag == 'gridSearch'){	// 점검항목 리스트
    		...
    		var result = response.sbeqpBatryInspItmMgmtList;
//    		addData  = { "batryId" : "", "inspStdDate" : sbeqpDtlLkup.calculateDate(0), "ouppInspStatCd" : "", "sfsTmpr" : "0"};
//    		result.unshift(addData);
	       	$('#'+gridId).alopexGrid('dataSet', response.sbeqpBatryInspItmMgmtList);
