var popMain = $a.page(function() {
    ...
    this.init = function(id, param) {
        ...
        if(param.sbeqpClCd == "AS"){
        	ivtrFlag = "Y";
        	$('#basicTabs').attr("style","display:none;");
        }
    	if(param.fix == "Y"){
			$('#sbeqpClCdList').setData({
				sbeqpClCdList:param.sbeqpClCd
			});
    		$('#sbeqpClCdList').attr("disabled",true);
    		$('#basicTabs').attr("style","display:none;");
    	}
    	 if(param.mtsoNm !="" ){
     		paramData = param;
    ...
    function setList(param){
    	if(JSON.stringify(param).length > 2){
        	...
        	if(paramData.sbeqpClCd =="AS"){
        		param.sbeqpClCdList = "A,S";
        	}else{
        		param.sbeqpClCdList = paramData.sbeqpClCd;
        	}
//        	param.fdfChk = "N";
    ...
    function setEventListener() {
    	var perPage = 100;
    	 ...
    	 $('#'+gridId).on('pageSet', function(e){
         	...
         	popMain.setGrid(eObj.page, eObj.pageinfo.perPage);
         });
    	...
    	//페이지 selectbox를 변경했을 시.
         $('#'+gridId).on('perPageChange', function(e){
         	...
         	popMain.setGrid(1, eObj.perPage);
         });

    	//조회
    	 $('#btnSearch').on('click', function(e) {
    		 popMain.setGrid(1,perPage);
         });

    	//엔터키로 조회
         $('#sbeqpLkupForm').on('keydown', function(e){
     		if (e.which == 13  ){
     			popMain.setGrid(1,perPage);
       		}
     	 });
...
//			httpRequest('tango-transmission-biz/transmisson/configmgmt/commoncode/bp', param,'GET', 'bp');
		...
		$('#btnExportExcel').on('click', function(e) {
			...
			var param =  $("#sbeqpLkupForm").getData();
		...
		$('#'+gridId).on('dblclick', '.bodycell', function(e){
			var dataObj = AlopexGrid.parseEvent(e).data;
			dataObj.eqpId = dataObj.sbeqpId;
			dataObj.eqpNm = dataObj.sbeqpNm;
			dataObj.eqpMdlNm = dataObj.sbeqpMdlNm;
			dataObj.eqpInstlMtsoNm = dataObj.sbeqpInstlMtsoNm;
			dataObj.eqpRoleDivNm = dataObj.sbeqpClNm;
			dataObj.endFlag = "Y";
			$a.close(dataObj);
		});
...
//			$a.close();
		...
		$('#basicTabs').on("tabchange", function(e, index) {
 			switch (index) {
 			case 0 :
 				$a.navigate('/tango-transmission-web/configmgmt/sbeqpmgmt/SbeqpEqpLkup.do');
 				break;
 			default :
 				break;
 			}
...
//  			}
    	...
    	if(flag == 'sbeqpClCd'){
    		$('#sbeqpClCdList').clear();
    		if(ivtrFlag == "Y") {
    			var option_data =  [{comCd: "A,S",comCdNm: "전체"}];
    		} else {
    		...
    		for(var i=0; i<response.length; i++){
    			var resObj = response[i];
    			if(ivtrFlag == "Y") {
        			if (resObj.comCd == "A" || resObj.comCd == "S"){
        				option_data.push(resObj);
        			}
    			}
    			else {
