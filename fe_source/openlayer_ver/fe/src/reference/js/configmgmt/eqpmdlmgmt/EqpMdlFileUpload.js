...
//    		var fileExtensionChk = this.value.toLowerCase();
//			if (fileExtensionChk.indexOf(".pptx") < 0 && fileExtensionChk.indexOf(".xlsx") < 0) {
//				alertBox('W', "확장자가 pptx 혹은 xlsx만 가능합니다.");
//				return;
//			}
    	...
    	$('#btnSave').on('click', function(e) {
    		...
    		callMsgBox('','C', configMsgArray['saveConfirm'], function(msgId, msgRst){
    			//저장한다고 하였을 경우
 		        if (msgRst == 'Y') {
 		        	$('#eqpMdlId').val(paramData.eqpMdlId);
...
// 		        	form.eqpMdlId = paramData.eqpMdlId;
 		        	var data = new FormData(form);
// 		        	data.eqpMdlId = paramData.eqpMdlId;

 		        	httpRequest('tango-transmission-biz/transmisson/configmgmt/eqpmdlmgmt/atflFileUpload', data, 'post', 'uploadFile');
 		        }
		     });
    	});
