...
$a.page(function() {
	...
	function setCombo() {
		...
		var param = {storgDivCd : "1", userId : userId};
    	httpRequest('tango-transmission-biz/transmisson/configmgmt/fhdsnobjmgmt/storgMgmtList', param, 'GET', 'gridSearch');
		var param = {storgDivCd : "2", userId : userId};
    	httpRequest('tango-transmission-biz/transmisson/configmgmt/fhdsnobjmgmt/storgMgmtList', param, 'GET', 'gridSearch2');
...
//     		, {key : 'fhBfChnlCardNm'    , align:'center', width:'90px', title : '채널카드'}
     		...
     		, {key : 'fhLtef800mSmtsoNm' , align:'center', width:'230px', title : '국소명'}
     		...
     		, {key : 'fhLtef26gSmtsoNm'  , align:'left', width:'90px', title : '국소명'}
...
//    		, {key : 'fhEngTmpId', align:'center', width:'90px', title : '상세ID', hidden : true}
...
//    		, {key : 'lttagVal', align:'center', width:'80px', title : '도'}
//    		, {key : 'lttmnVal', align:'center', width:'80px', title : '분'}
//    		, {key : 'lttscVal', align:'center', width:'80px', title : '초'}
//    		, {key : 'ltdagVal', align:'center', width:'80px', title : '도'}
//    		, {key : 'ltdmnVal', align:'center', width:'80px', title : '분'}
//    		, {key : 'ltdscVal', align:'center', width:'80px', title : '초'}
//    		, {key : 'latVal', align:'center', width:'100px', title : '위도'}
//    		, {key : 'lngVal', align:'center', width:'100px', title : '경도'}
    ...
    function setEventListener() {
    	...
    	$('#search').on('click', function(e) {
        	...
        	dataParam.rowPerPage = '500';
	   		var hdofc = $("#hdofcCd").getTexts();
	   		if(dataParam.hdofcCd == "" || dataParam.hdofcCd  == null ){
	        	dataParam.hdofcNm = "";
	   		}else{
    	...
    	$('#btnFhTmpInsert').on('click', function(e) {
			...
			if(storgId == '' || storgId == null) {
		    	...
		    	if (checkCd == "T" && (dataParam.upStorgId == "" || dataParam.upStorgId == null || dataParam.upStorgId == undefined)) {
		    		alertBox('W', '목록 전체 선택시에는 업로드 그룹을 지정하셔야 등록가능합니다.');
		    	} else {
		    		callMsgBox('','C', '정말 등록하시겠습니까?', function(msgId, msgRst){
			    		...
			    		if (msgRst == 'Y') {
			    			bodyProgress();
			    			if (checkCd == "F") {
			    				dataParam.fhEngTmpIdList = idTmp;
			    			}
			        		...
			        		if (checkCd == "F") {
			        			httpRequest('tango-transmission-biz/transmisson/configmgmt/fhdsnobjmgmt/insertTmpExcel', dataParam, 'POST', 'insertlist');
			    			} else {
			    				httpRequest('tango-transmission-biz/transmisson/configmgmt/fhdsnobjmgmt/insertTmpExcelAll', dataParam, 'POST', 'insertlist');
			    			}
			    		}
			    	});
		    	}
    	...
    	$('#btnFhTmpList').on('click', function(e) {
    		var storgId = $("#storgId").val();
    		var param = {storgDivCd : "1", storgId : storgId};
    		$a.popup({
	        	popid: 'FhTmpPop',
	        	title: 'F/H ',
	        	//iframe: true,
    	...
    	$('#btnFhTmpMaster2').on('click', function(e) {
    		var dataParam =  $("#searchForm").getData();
    		dataParam.storgDivCd = "2";
    		$a.popup({
	        	popid: 'FhTmpLockerMgmtPop',
	        	title: '업로드그룹관리',
	        	//iframe: true,
	            ...
	            ,callback: function(resultCode) {
	            	...
	            	var param = {storgDivCd : "2", userId : userId};
	            	ckData = resultCode;
	            	httpRequest('tango-transmission-biz/transmisson/configmgmt/fhdsnobjmgmt/storgMgmtList', param, 'GET', 'gridSearch3');
...
//	        	modal : true,
	        	...
	        	*/
	            ,callback: function(resultCode) {
	            	if (resultCode != null) {
	            		$("#upStorgId").setSelected(resultCode);
		            	//
		            	$('#search').click();
	            	}
//	            	$("upStorgId").setData({
//	            		upStorgId: resultCode
//	            	});
//	            	setTimeout($('#search').click(),3000);
//                	if (resultCode == "OK") {
//
//
//                	}
...
//    		$("#hdofcNm").setEnabled(false);
    	...
    	if(flag == 'gridSearch3'){	// 등록
    		var tmpUpStorgId = $("#upStorgId").val();
			$("#upStorgId").clear();
    		var option_data =  [{cd: "",cdNm: "선택하세요"}];
    		for(var i=0; i<response.storgMgmtList.length; i++){
    			var resObj = response.storgMgmtList[i];
...
//
//        		if (tmpUpStorgId != "") {
//        			if (option_data2.indexOf(tmpUpStorgId) != -1) {
//        				alert("Reload");
//    				} else {
//    					alert("X");
//    				}
