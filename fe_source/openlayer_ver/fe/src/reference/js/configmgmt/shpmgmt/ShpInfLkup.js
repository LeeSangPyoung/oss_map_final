    	...
    	//수정
    	 $('#ShpGridRack').on('dblclick', '.bodycell', function(e){
    		var dataObj = null;
       	 	...
       	 	gridId = "ShpGridRack";
    		popup('RackReg', $('#ctx').val()+'/configmgmt/shpmgmt/RackReg.do', '형상 Rack 수정', dataObj);
         });
    	 ...
    	 $('#ShpGridShlf').on('dblclick', '.bodycell', function(e){
    	 	...
    	 	if (skbYn == 'Y')
    	 	 ...
    	 	 gridId = "ShpGridShlf";
     		 popup('ShlfReg', $('#ctx').val()+'/configmgmt/shpmgmt/ShlfReg.do', '형상 Shelf 수정', dataObj);
          });
    	 ...
    	 $('#ShpGridCard').on('dblclick', '.bodycell', function(e){
    	 	...
    	 	if (skbYn == 'Y')
    	 	 ...
    	 	 gridId = "ShpGridCard";
    	 	popupCard('CardReg', $('#ctx').val()+'/configmgmt/shpmgmt/CardReg.do', '형상 Card 수정', dataObj);
          });
...
//      		 popup('PortReg', $('#ctx').val()+'/configmgmt/portmgmt/PortReg.do', '형상 Port 수정', dataObj);
    	...
    	//등록
    	 $('#btnRegRackLkup').on('click', function(e) {
    		...
    		popup('RackRegLkup', $('#ctx').val()+'/configmgmt/shpmgmt/RackReg.do', '형상 Rack 등록', dataParam);
    	 ...
    	 $('#btnRegShlfLkup').on('click', function(e) {
    		 dataParam = {"fromShlfReg" : "Y", "eqpId":$('#eqpIdShpInf').val(), "eqpNm":$('#eqpNmShpInf').val(), "intgFcltsCd":$('#intgFcltsCdInf').val(), "eqpRoleDivCd":$('#eqpRoleDivCdInf').val()};
     		 popup('ShlfRegLkup', $('#ctx').val()+'/configmgmt/shpmgmt/ShlfReg.do', '형상 Shelf 등록', dataParam);

          });

    	 $('#btnRegCardLkup').on('click', function(e) {
    		 dataParam = {"regYnCard" : "N", "fromCardReg" : "Y", "eqpId":$('#eqpIdShpInf').val(), "eqpNm":$('#eqpNmShpInf').val(), "intgFcltsCd":$('#intgFcltsCdInf').val(), "eqpRoleDivCd":$('#eqpRoleDivCdInf').val(), "mgmtGrpNm":paramData.mgmtGrpNm}; //[20171221]
    		 popupCard('CardRegLkup', $('#ctx').val()+'/configmgmt/shpmgmt/CardReg.do', '형상 Card 등록', dataParam);
    	 ...
    	 $('#btnCardBarNoMapp').on('click', function(e) {
       		var  param = $("#shpInfLkupForm").getData();
	       		 ...
	       		 $a.popup({
		 	          	...
		 	          	title: '카드 바코드 매핑',
		 	            url: $('#ctx').val()+'/configmgmt/shpmgmt/CardBarNoMappPop.do',
		 	            modal: true,
		                 movable:true,
		                 data:param,
		                 windowpopup: true,
       		...
       		var  param = _.extend($("#shpInfLkupForm").getData(), {
    			eqpId : $('#eqpId').val()
	       		 ...
	       		 $a.popup({
		 	          	...
		 	          	title: '장비구간 생성 팝업',
		 	            url: $('#ctx').val()+'/configmgmt/equipment/EqpSctnInfCreatePop.do',
