// Drawing 패키지 - AreaDrawPolygonService
// 담당 기능: 다각형 영역 그리기
...
export interface AreaDrawPolygonOptions {
  style?: {
    color: string;
    weight: number;
    fill: boolean;
...
export class AreaDrawPolygonService {
  ...
  // 다각형 영역 그리기 모드 활성화
  activateAreaDrawPolygonMode(options?: AreaDrawPolygonOptions): void {
    try {
      if (!this.map) {
      ...
      console.error('Area Draw Polygon 모드 활성화 중 오류:', error);
      throw error;
    }
  }

  private createStyleArea(options?: AreaDrawPolygonOptions) {
    return new Style({
      stroke: new Stroke({
  ...
  private startDraw(options?: AreaDrawPolygonOptions) {
    if (!this.map) return;
    ...
    draw.on('drawend', event => {
      ...
      event.feature.set('mode', 'polygon-area-draw');
