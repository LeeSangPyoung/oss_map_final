...
const getFeaturesFromBrowser = (map: any, pixel: number[]) => {
  try {
    if (!map || !pixel) {
      console.log('❌ 맵 또는 픽셀 정보 없음');
      return [];
    }

    console.log('🔍 getFeaturesAtPixel 호출 중...');
    // 브라우저 내에서 피처 검색
    const features = map.getFeaturesAtPixel(pixel, {
      hitTolerance: 15 // 15픽셀 반경으로 검색 (경계 문제 해결)
    });

    console.log('🔍 getFeaturesAtPixel 결과:', features ? features.length : 0, '개');
    ...
    const convertedFeatures = features.map((feature: any) => {
      ...
      // ID 디버깅 및 수정
      console.log('🔍 Feature ID 디버깅:', {
        originalId: geoJSON.id,
        hasGetId: !!feature.getId,
        getIdResult: feature.getId ? feature.getId() : 'no getId method'
      });
      
      // ID가 없으면 feature.getId()로 가져오기
      if (!geoJSON.id && feature.getId) {
        geoJSON.id = feature.getId();
        console.log('🔧 ID 수정됨:', geoJSON.id);
      }
      ...
      if (!geoJSON.id && geoJSON.properties) {
        if (geoJSON.properties.id) {
          ...
          console.log('🔧 Properties에서 ID 추출:', geoJSON.id);
        } else if (geoJSON.properties.fid) {
          geoJSON.id = geoJSON.properties.fid;
          console.log('🔧 Properties에서 FID 추출:', geoJSON.id);
        }
      }
    ...
    console.log('🔍 변환된 피처:', convertedFeatures.length, '개');
    
    // ID가 있는 피처만 필터링
    const validFeatures = convertedFeatures.filter(feature => {
      const hasValidId = feature.id && feature.id !== 'undefined' && feature.id !== undefined;
      if (!hasValidId) {
        console.log('❌ ID가 없는 피처 필터링됨:', feature);
      }
      return hasValidId;
    });
    
    console.log('✅ 유효한 ID를 가진 피처:', validFeatures.length, '개');
...
export const getListFeaturesInPixel = async (
  ...
  if (useBrowserSearch && map && clickPixel) {
    ...
    if (browserFeatures.length > 0) {
      ...
      const clickableFeatures = validFeatures.map(feature => {
        try {
          ...
          if (isInsideAnyPolygon && (feature.geometry.type === 'Point' || feature.geometry.type === 'LineString' || feature.geometry.type === 'MultiLineString')) {
            let debugThreshold;
            if (feature.geometry.type === 'Point') {
              debugThreshold = useBrowserSearch ? 3 : 5; // Hover는 3픽셀, 클릭은 5픽셀 (조금 완화)
            } else if (feature.geometry.type === 'LineString') {
              debugThreshold = 20;
            } else if (feature.geometry.type === 'MultiLineString') {
              debugThreshold = 20;
      ...
      console.log('📊 클릭 가능한 피처:', clickableFeatures.length, '개');
      if (clickableFeatures.length > 0) {
        ...
        clickableFeatures.sort((a, b) => {
          ...
          if (a.priority === 3 && b.priority === 3) {
            ...
            if (areaRatio > 0.9) {
              console.log('📏 크기가 비슷한 폴리곤들 - 거리 기반 선택');
              return a.pixelDistance - b.pixelDistance;
            } else {
              // 크기가 크게 다른 폴리곤들 → 작은 폴리곤 우선
              console.log('📏 크기가 다른 폴리곤들 - 작은 폴리곤 우선');
        ...
        console.log('🎯 최종 선택:', {
          ...
          type: selected.feature.geometry.type,
          priority: selected.priority,
          distance: selected.pixelDistance
        });
        
        // 모든 클릭 가능한 피처 로그
        console.log('📋 모든 클릭 가능한 피처:');
        clickableFeatures.forEach((item, index) => {
          const areaInfo = item.feature.geometry.type === 'Polygon' ? `, 면적: ${item.area.toFixed(2)}` : '';
          console.log(`  ${index + 1}. ${item.feature.id} (${item.feature.geometry.type}) - 우선순위: ${item.priority}, 거리: ${item.pixelDistance}${areaInfo}`);
        });
  ...
  const promises = layerData?.map(async layer => {
    ...
    console.log(`🌐 WFS 요청: ${layer.value}`);
  ...
  if (allFeatures.length > 0) {
    ...
    // 디버깅을 위한 로그 추가
    console.log('WFS에서 가져온 모든 features:', allFeatures);
    console.log('Features 개수:', allFeatures.length);
