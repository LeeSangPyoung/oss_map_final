...
export const useAdvancedSelect = (options: UseAdvancedSelectOptions) => {
  ...
  const activate = useCallback(async () => {
    ...
    // Advanced Select 활성화 시 즉시 Vector 레이어 생성
    try {
      const coordinate = map.getView().getCenter();
      const zoom = map.getView().getZoom() || 0;
      ...
      if (coordinate && Array.isArray(coordinate) && coordinate.length >= 2) {
        ...
        console.log('AdvancedSelect: 활성화 시 Vector 레이어 즉시 생성');
        await loadAllFeaturesToVectorLayer(map, currentLayerData, coordinate, zoom);
        console.log('AdvancedSelect: 활성화 시 Vector 레이어 생성 완료');
      }
    } catch (error) {
      console.error('AdvancedSelect: 활성화 시 Vector 레이어 생성 오류:', error);
    }
  }, [map, handleMapClick, handleMapMouseMove, handleMapMove, layerData]);
