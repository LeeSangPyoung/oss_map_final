  ...
  private handleMapMove = async (event: any): Promise<void> => {
    ...
    setTimeout(async () => {
      try {
        ...
        console.log('AdvancedTrailDrawPolygon: 맵 이동 후 Vector 레이어 재생성 시작');
        await loadAllFeaturesToVectorLayer(this.map, layerData, coordinate, zoom);
        console.log('AdvancedTrailDrawPolygon: 맵 이동 후 Vector 레이어 재생성 완료');
      } catch (error) {
        console.error('AdvancedTrailDrawPolygon: 맵 이동 후 all-features-layer 업데이트 실패:', error);
      }
    }, 500);
  };
    ...
    try {
      ...
      if (vectorLayer) {
        ...
        if (layerData && layerData.length > 0) {
          ...
          if (coordinate) {
            try {
              console.log('AdvancedTrailDrawPolygon: 초기 Vector 레이어 생성 시작');
              await loadAllFeaturesToVectorLayer(this.map, layerData, coordinate, zoom);
              this.allFeaturesLayerCreated = true;
              console.log('AdvancedTrailDrawPolygon: 초기 Vector 레이어 생성 완료');
            } catch (error) {
              console.error('AdvancedTrailDrawPolygon: 초기 Vector 레이어 생성 실패:', error);
            }
          }
        }
      }
