// Drawing 패키지 - AreaDrawCircleService
// 담당 기능: 원형 영역 그리기
...
export interface AreaDrawCircleOptions {
  style?: {
    color: string;
    weight: number;
    fill: boolean;
...
export class AreaDrawCircleService {
  ...
  // 원형 영역 그리기 모드 활성화
  activateAreaDrawCircleMode(options?: AreaDrawCircleOptions): void {
    try {
      if (!this.map) {
      ...
      console.error('Area Draw Circle 모드 활성화 중 오류:', error);
      throw error;
    }
  }

  private createStyleArea(options?: AreaDrawCircleOptions) {
    return new Style({
      stroke: new Stroke({
  ...
  private startDraw(options?: AreaDrawCircleOptions) {
    if (!this.map) return;
    ...
    draw.on('drawend', event => {
      ...
      event.feature.set('mode', 'circle-area-draw');
